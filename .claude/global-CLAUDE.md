...
## 사용자 조작 안내 (PeterJ가 직접 해야 하는 것)
- PeterJ가 직접 해야 하는 조작(설정 변경·토큰/키 발급·계정 인증·앱/웹 UI 탐색 등)은
  **아주 상세한 스텝바이스텝**으로 안내한다: 어느 화면에서 **무엇을 누르고, 어디로 스크롤하고,
  어떤 항목·라벨을 고르는지**까지 구체적으로. "설정에서 바꾸세요" 식 뭉뚱그림 금지
  (PeterJ는 모바일/원격 위주라 화면 맥락 없이는 따라가기 어렵다).
- **안내 전에 그 앱·사이트의 "현재 최신" 화면·메뉴 구성을 먼저 확인**한다(공식 문서·웹 검색·
  스크린샷 등으로 근거 확보). 서비스 UI는 자주 바뀌므로 **기억 속 옛 화면으로 추측하지 말고**,
  PeterJ가 쓰는 **기기 폼(모바일 앱 / 모바일 웹 / 데스크탑)에 맞는 실제 현재 레이아웃을
  근거로** 단계를 구성한다.
- 확인이 안 되거나 화면을 직접 볼 수 없으면 **추측을 사실처럼 쓰지 말고** 확인된 부분과
  추정 부분을 구분해 표기하고, 필요하면 스크린샷을 요청해 실제 화면에 맞춰 안내한다.
