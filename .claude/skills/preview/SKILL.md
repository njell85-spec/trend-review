...
2. `node scripts/preview.mjs` 실행 (아무 cwd에서나 가능) →
   `output/preview-phone.png`(390px) · `output/preview-tablet.png`(800px)
   + 카톡 메시지 **포맷** 미리보기 텍스트 출력.
3. 스크린샷 2장과 (포맷 변경 시) 카톡 미리보기를 PeterJ에게 보여준다.
   파일 전송 도구가 있으면 이미지로 전송한다.
