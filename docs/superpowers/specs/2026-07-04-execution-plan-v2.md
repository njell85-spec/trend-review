...
- 작성: 2026-07-04, Fable 5 세션 · 상태: **적용 완료 (실험 트랙 G4·G5 제외 — PeterJ 승인, 2026-07-04)**
- 적용 내역: G1(전역 지침 개정) · G2(부트스트랩 일원화+경쟁 조건 수정) · T1(/preview) · T2(spec-lint+CI) · T3(품질 게이트) · T4(브랜치 정리)
- 커밋 전 4렌즈 적대적 리뷰(에이전트 15개)로 결함 10건 확정·전량 수정 후 재검증(spec-lint 미탐 6종 시나리오 10/10 적발, preview 외부 cwd 실행, bootstrap 로컬 우선/다운로드 양 경로). G2의 "프로젝트 원본 우선" 로직은 훅 병렬 실행 경쟁(리뷰 확정 major)을 반영한 설계 변경.
