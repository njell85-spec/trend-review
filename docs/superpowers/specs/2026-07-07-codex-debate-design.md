...
## 12. 설계토론 모드 (2026-07-11 개정)

- **트리거**: "클코덱스 설계토론 [주제]". 골격(독립 R0 → 변증 R1..N 캡4 → 심판 수렴 →
  승인 게이트)은 코드리뷰 모드와 동일, 대상만 코드 diff → 설계 주제/스펙 초안.
- **R0**: 양측이 독립 설계 초안(요구사항 해석·아키텍처·트레이드오프·리스크·미해결 질문)
  작성. codex는 기존 `codex-review.sh` 재사용(프롬프트만 설계용), Claude 측은
  general-purpose 서브에이전트.
- **심판**: `review-judge` 프롬프트를 설계 판정용(합의/쟁점/판정/남은 질문)으로 변형.
- **산출**: 합의 설계안을 `docs/superpowers/specs/<날짜>-<슬러그>-design.md` 초안(draft
  표기)으로 저장. PeterJ 확정 전 구현 착수·기존 스펙 덮어쓰기 금지.
- **배경**: EMR_Assist_v1 `docs/productivity/2026-07-11-fable-parity-playbook.md` §3-B —
  큰 아키텍처 결정에서 이종 모델 2개 + 심판이 단일 상위 모델보다 맹점을 잘 잡는다는
  원리를 설계 단계로 확장.
