...
### 누적 표도 페이지별로 나눈다 — **과거 행 포함** (PeterJ 확정 2026-08-06)

카드뿐 아니라 **하단 누적 표도 페이지별로 가른다. 이전 리스트까지 전부 옮긴다.**
```
index.html       누적 표 → 논문 행만
guidelines.html  누적 표 → 📋 가이드라인 행 + 🔖 기타 행 (섹션별)
```

**★ 과거 행은 상태 파일이 아니라 배포된 HTML에서 가져와야 한다 (실측 제약).**

| 소스 | 건수 | 저널명 |
|---|---:|---|
| `output/selected_papers.json` | 38 | **없음** — 필드가 `{pmid, title, date}` 뿐 |
| `output/analysis_archive.json` | 29 | 있음 |
| 현재 `index.html` 표 행 | 38 | **전부 있음**(발행 시점에 박혔다) |

→ 상태 파일만으로 재생성하면 **과거 9건의 저널명이 소실된다**(38 − 29). 따라서 페이지 분할 시
**일회성 마이그레이션**으로 현재 `index.html`의 표 행을 파싱해 두 페이지로 나눠 옮긴다.
데이터 손실 0.

가르는 키는 **이미 마크업에 있다** — 가이드라인 행에만 `data-guideline="1"`이 붙는다
(`GitHubPublisher.js:452`). 그 속성 유무로 갈라 넣으면 된다.

이후 신규 행은 각 페이지의 발행 경로가 자기 표에 추가한다. 읽음 체크박스는 `pmid`/`sourceId`
키의 localStorage라 **페이지가 갈려도 체크 상태가 따라간다**.

> **주의 — §4-3(전량 재생성)과의 관계**: `guidelines.html`은 상태 파일에서 전량 재생성하되,
> **최초 1회 표 마이그레이션만은 HTML 파싱**이다. 이후에는 상태 파일이 정본이 되도록
> 가이드/기타 상태 파일에 `journal` 필드를 추가해 기록한다(현행 `appendState`가
> `{pmid, title, date}`만 쓴다 — `scripts/on-demand.mjs:145-156`). **이 필드 추가를 빠뜨리면
> 다음 재생성에서 같은 소실이 반복된다.**
...
| 8-3 | 대시보드 v2 경계선 | **구/신 구간 통계 분리까지** (3-2) | 경계선은 `index.html`의 **논문 표** 안. 아래 §8-3 상술 — 단 **평균 점수 비교는 불가**(스케일 상이)라 구성비 지표로 낸다 |
| 8-5 | 누적 표 페이지 분할 | **과거 행 포함해 전부 나눈다** | §5.5-B "누적 표도 페이지별로" — 일회성 HTML 파싱 마이그레이션(상태 파일엔 저널이 없어 재생성 불가). 상태 파일에 `journal` 필드 추가가 후속 필수 |
...
**★ 평균 rawScore 는 비교 지표로 쓰지 않는다. 스케일이 다르다.**
```
v1  저널 4.0 + 주제 4.0 + 설계 1.5 + 최신 0.7 + 표본 0.8  =  최대 11.0
v2  저널 4.0 + 주제 4.0 + 설계 2.0                        =  최대 10.0
```
축 구성이 바뀌었으므로 `v1 평균 7.74 vs v2 평균 8.9` 식의 대조는 **다른 자를 두 개 대는 것**이다.
나란히 놓으면 개선/악화를 거꾸로 읽게 만든다. → **구성비 지표만 낸다**(스케일 무관, 정직하게 비교됨).

| 지표 | 비고 |
|---|---|
| **건수 n** | **항상 병기 필수** — v2 초기엔 n=5~10이라 구성비가 크게 흔들린다. n이 보여야 "표본이 적다"를 읽을 수 있다 |
| 저널 티어 분포 | top / flagship / specialty / default / **low** 각 % |
| 설계 분포 | RCT·메타 / 관찰 / **서술형 리뷰** 각 % |
| 단일 주제 최대 점유율 | v1 = 패혈증 64% (확정 ③ 목표 30% 미만) |
| 간호지·low 티어 픽 수 | v1 = F2로 다수 · v2 목표 **0** |
| **LLM 재순위 적용률** | v1 = **0%**(F1으로 한 번도 안 돎) · v2 ≈ 100%. 가장 극적인 대조 |

**v1 구간 통계 산출 비용 — 일회성 PubMed 백필이 필요하다.**
과거 38편의 **설계 라벨(RCT/리뷰 등)이 어디에도 저장돼 있지 않다** —
`selected_papers.json`은 `{pmid, title, date}`뿐이고 `analysis_archive.json`(29건)에도 설계는 없다.
→ 38개 PMID를 `fetchArticles`로 1회 재조회해 저널·PublicationType·MeSH를 받아
v1 구성비를 계산한다. efetch 4배치(BATCH=10), 일회성 스크립트 ~50줄.
산출물은 `output/score_version_stats.json`에 고정 저장하고 대시보드가 그걸 읽는다
(매 발행마다 재조회하지 않는다 — 과거 구간은 불변이다).

**v2 구간**은 §5.4의 선정 증거 영속화(`scoreVersion`·`streamSource`·`primaryTopic`·
`rerankApplied`)가 이미 필요한 필드를 전부 남기므로 **추가 조회 없이** 집계된다.
이것이 3.4를 P1-4에 넣어야 하는 또 하나의 이유다.
