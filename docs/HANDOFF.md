...
**상태**: D1~D8 완료(§5). Google Secrets 4종 실작동 확인. **다음 = 오후 모바일 M1~M4.**
남은 코드 액션은 아카이브 pmid 실버그 PR 병합(§5, CI 초록 확인 후) 하나뿐.

0. **아카이브 pmid 실버그 PR 병합(선행 권장)**: 브랜치 `claude/d8-phase2-validation-pxeu3h`
   — CI(spec-lint+test:unit 35건) 초록 확인 후 main 병합. 데일리 아카이브가 켜지기 전
   (또는 재검증 전) 병합해 두면 같은 날 항목 유실·OA 중복 스킵을 원천 차단.
1. ~~D8 — Phase 2 실동작 검증~~ **완료**(§5, run `28769877446`, SOHO/PMID 41841715).
   재검증이 필요하면 같은 PMID로 재트리거 → 항목·Drive 파일 중복 없이 upsert되는지 확인
   (단 on-demand는 재실행마다 카톡을 재발송하므로 PeterJ 양해 필요).
