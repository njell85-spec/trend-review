...
>
> **[2026-08-06 — 논문 선정 품질 진단 완료(F1~F7) + 설계토론 R0 절반 · 코드 무변경]**
> - **성격**: 진단·설계 단계. **프로덕션 코드·config 한 줄도 안 건드렸다.** 승인 게이트 준수 중.
>   작업 브랜치 `claude/paper-selection-quality-2221ez`(main과 동일 베이스 `d8ffc2c` + 문서 커밋만).
> - **먼저 읽을 것: `docs/reviews/2026-08-06-1421-selection-quality-debate.md`** — 진단 F1~F7
>   전문 + Fable R0 설계 초안 전문이 여기 있다. 아래는 요약이니 그 문서가 정본.
> - **★ F1 = 최대 발견: LLM 재순위(3층)가 프로덕션에서 한 번도 돈 적 없다.**
>   `daily-review.yml:85` `RERANK_POOL: ${{ vars.RERANK_POOL }}` → Variable 미설정이라 **빈 문자열**
>   주입 → `Number(process.env.RERANK_POOL ?? 20)`에서 `??`가 빈 문자열을 통과시켜 `Number('')=0`
>   → `poolSize=max(1,0)=1` → `_rerankSelect(pool=1,n=1)`이 `pool.length<=n`에서 **LLM 없이 즉시 반환**.
>   실측(run 31053207293): ANALYZING **0.1초**, `LLM 실행 경로: 구독×1`(그 1회는 PICO).
>   로그는 플래그만 보고 `(LLM reranked)`를 찍어 **4주간 은폐**. `selection-experiment.yml`은
>   `RERANK_POOL`을 아예 안 넘겨 `undefined ?? 20`→20으로 정상 동작 — **실험만 되고 프로덕션만
>   안 되던** 구조(07-10 브랜치 검증이 초록이었던 이유). → 최근 25편은 전부 순수 결정적 1등.
> - **F2** `journals.json` flagship `includes`의 맨몸 `"critical care"`가 7종을 3.2로 승격
>   (간호지 2종·Current opinion·explorations·J crit care·crit care science 포함). `"jacc"`는
>   **J. Am. Coll. Clinical Pharmacy**를 오매칭. **라이브 상위 20 중 11편이 이 경로**, 간호지가 3·9·11·12위.
> - **F3** 주제 포화(제목 관심어 2개면 만점) — 25편 중 10편이 정확히 4.00. 수집 쿼리가 MeSH 3개뿐이라
>   선정 25편 중 22편의 MeSH에 검색어가 그대로 박힘(동어반복). ※ 주제게이트(-5)는 **정상 작동**
>   (라이브 하위 5편 전부 GATED) — 무관 논문만 거를 뿐 관심 범위 안 변별엔 무기여.
> - **F4** 설계 축 무력(RCT 1.50 vs 서술형 리뷰 0.64 = 0.86점차 vs 저널 4.2점 스윙). 라이브 상위 20에
>   Review 6·Meta 4, RCT 사실상 없음. **F5** 표본 추출이 쓰레기 숫자(`N≈1704632`). **F6** 최신성이
>   사실상 상수(0.14점) + `JournalIssue.PubDate`를 읽어 2026-07 공개분이 "1년+"로 판정.
>   **F7** 수집 실효창이 180일이 아니라 **~2주**(3561편 중 sort=date로 300편) → 25일간
>   NEJM·Lancet·JAMA·ICM·Resuscitation·Annals EM **0편**, 16/25가 패혈증.
> - **근거 실행**: `selection-experiment` run **31072871934**(EXP_LLM=0, 프로덕션 무영향, success)로
>   라이브 300편 풀 상위 20 확보. 축별 분해는 PubMed MCP로 25편 메타 실조회 후 재계산.
> - **설계토론(클코덱스 설계토론, PeterJ 지정 "Fable vs 5.6 Sol")**: Claude측 **Fable R0 완료**,
>   codex측 **미착수** — `CODEX_AUTH_B64`가 빈 값이라 `codex login status`=`Not logged in`.
>   부트스트랩 로직은 정상(`[ -n "$CODEX_AUTH_B64" ]` 가드에 걸려 스킵). **2026-08-06 PeterJ가
>   데스크탑에서 값 재저장 완료** → 다음 세션에서 `codex login status`로 실측 후 R0 이어가면 된다.
> - **Fable R0 요지**: 주범은 F7(천장)×F2(오염)×F1(안전망) 곱, 하나만 꼽으면 **F1**.
>   **3층 아키텍처는 무죄**(300편 전량 LLM 스크리닝 전환 반대). 제안 = 수집 이중 스트림(최신 + 저명저널
>   `[ta]` 화이트리스트) · 쿼리에서 `"sepsis"[MeSH]` 제거 · 저널 부분일치→exact(ISSN은 반대) ·
>   주제 탈포화 · 설계 축 증폭+서술형 리뷰 강등 · **표본·최신성 축은 수리 말고 삭제** ·
>   **누적 38편 리스트 폐기 반대**("나쁜 픽을 다시 뽑을 권리뿐") · 리플레이 하네스로 품질 계측.
>   PeterJ 확인 필요 4건(리뷰 지위 / 간호지 처분 / 패혈증 목표 점유율 / 따라잡기 기간 허용)은 그 문서 §2.10.
> - **부수 확인 — on-demand URL 경로**: 워크플로·스크립트·publisher는 main에 **있다**(PR #65).
>   **갭**: 대시보드 위젯 "직접 입력"이 `/^\d{5,9}$/`·DOI만 통과시켜 **URL을 거부**한다
>   (`GitHubPublisher.js`). 폰에서 가이드라인 URL을 못 넣고 Actions 수동 실행해야 함. 수정은 소품
>   (정규식 + kind 강제). **대시보드 변경이라 push 전 `/preview` 승인 대상.**
> - **PeterJ 신규 안건(미착수·브레인스토밍 필요)**: 가이드라인을 **별도 페이지 + 별도 누적 관리**로
>   분리. 세 갈래 — (가)매일 1건 신규만 (나)적절한 것 있을 때만 (다)**2~3개년치 풀을 미리 만들어
>   하루 하나씩 소진하며 풀 관리**. (다)는 "선정"이 아니라 **큐 관리** 문제라 성격이 다름.
>   선정 개편과 한 묶음으로 설계할 것(둘 다 "매일 뭘 고를 것인가" 문제).
> - **남은 것**: ① codex R0 → 변증 → 심판 → 합의 스펙 ② 승인 후 구현 ③ 위젯 URL 갭
>   ④ 가이드라인 트랙 브레인스토밍 ⑤ (기존) 아카이브 하루 지연 버그 ⑥ (기존) PR #64는
>   **GOOGLE_REFRESH_TOKEN 재발급 전 머지 금지** — 데일리가 매일 `invalid_grant` 경고 중.
>
