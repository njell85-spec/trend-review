...
> 최종 갱신: 2026-07-07 (KST) 낮 · **세션 크래시 복원**.
...
5. **TR master plan** (통합 계획서 Artifact, HTML): **v16** = https://claude.ai/code/artifact/757a28f8-bef7-4d5d-bc38-0dbccf747a5f
   — 한눈 지도(Phase 1→4 보드 + R/A/B) + 현황(완료/남은/주의/결정) + 코드 상세 + 누적 버전 기록.
   **★ 이름·운영 규칙(PeterJ 확정 2026-07-07 — 반드시 준수)**:
   - **공식 이름 = "TR master plan"**. PeterJ가 **TR plan / 로드맵 / 마스터플랜 / 서머리 / sum** 중
     무엇으로 부르든 **전부 이 문서**를 가리킨다. 예: "현재까지 내용 sum에 업데이트해서 올려줘"
     = 이 Artifact를 최신화해 같은 URL에 재배포 + 버전 올림.
   - **버전 누적**: 갱신 때마다 문서 안 "버전 기록" 맨 위에 새 줄 추가(옛 줄 삭제 금지), 라벨도 올림
     (현재 v16 — 한눈 지도·R/A/B 코드·완료/남은 것·버전기록 + 바로가기 링크 2종). 같은 URL 유지(구버전은 버전 선택기 + 문서 내 기록에 보존).
   - **작업 후 리추얼**: 규모 있는 작업을 마치면 이 문서를 갱신해 **리포트처럼 전달**(PeterJ가
     sum으로 받아봄). 코드(R·A·B)는 유지하되 문서에 범례/지도가 함께 있어 해석 가능해야 한다.
   - 코드 체계 해설: R=로드맵(큰 작업)·A=자동(세션)·B=PeterJ몫. 옛 코드 P(우선순위)=R을 급한 순,
     M1~4=B1~4, D1~8=데스크탑 설정(완료). 이 매핑을 문서 "옛 코드 대응" 섹션에 유지.
