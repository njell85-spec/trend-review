...
>
> **[2026-08-29 — 2026 심부전 지침 4건 발행 + 종합 카드(kind=synthesis) 신설 · PR #147]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.** 아래는 정답이 아니라 대조 기준선이다.
>
> **■ 무엇을 했나**
> ```
>   1) 2026 심부전 지침 4건을 ▶(on-demand)로 발행 — 가이드라인 18~21
>      · 2026 ESC 심부전 관리 지침        (URL 직접 지정 — PubMed 미색인이라 PMID 가 없다)
>      · 2022 AHA/ACC/HFSA 심부전 관리 지침
>      · 제2차 심부전 보편적 정의 2026    (PMID 42366993)
>      · 2026 ACC HFpEF 결정경로          (PMID 42494134)
>   2) kind=synthesis 신설 — 문헌 2~5건을 한 장으로 대조하는 카드 (PR #147)
>   3) 첫 실전 발행 — 가이드라인 23. PeterJ 지시로 2022 는 빼고 2026년 3종만 대조.
>      run 33250759773 · 커밋 24ff0dd · verify-pages 통과 · 가이드라인 23건
> ```
>
> **■ ★ 새 섹션 마커를 만들지 않았다 — GSECTION 재사용**
> ```
>   발행 가능한 카드가 셋뿐이고(SECTION·GSECTION·RSECTION) on-demand 는 문서를 하나만
>   받았다. "문헌 N건 → 카드 1장" 이 들어갈 자리가 없던 것이지, 형식만 있으면 되는 일이었다.
>   ★ 새 마커를 만들면 병합·페이지분할·무손실 렌더 검사를 전부 건드려야 한다 —
>     이 저장소는 바로 그 자리에서 카드를 통째로 날린 적이 있다(GSECTION 8→7).
>     그래서 GSECTION 을 그대로 쓰고 섹션 키만 다르게 했다.
>   기성 비교 문헌으로 때우는 길은 막혀 있었다: 실측한 비교 논문이 전부 ESC 2021 기준이라
>   2026 판 변경점(HFmrEF 폐지·stage A~D·GDMT 명칭 폐기)이 통째로 빠진다.
> ```
>
> **■ ★★ 오귀속은 프롬프트가 아니라 구조로 막는다 (이 카드의 제1 실패 모드)**
> ```
>   제1 실패 = "ESC 가 이렇게 말했다" 가 실은 합의문 내용인 것. 화면에서는 정상으로 보이고
>   원문을 다시 읽어야 알아챈다 — 즉 사람이 못 잡는 부류다. 두 겹으로 막았다:
>     ① 스키마 — refId 를 그 실행의 실제 문헌 목록으로 enum 고정
>     ② 매핑·렌더 — 모르는 refId 는 버리고, 입장이 전부 버려진 축은 축째 버린다
>        (상태 파일의 구판·외부 데이터가 매핑을 우회해 바로 렌더되는 경로까지 막았다)
>   실질 게이트도 하나 뒀다: 축이 없거나 모든 축이 단일 입장이면 대조가 아니라 요약 재탕이다.
>   1회만 다시 시킨다 — 숫자가 정상처럼 보이는 고장이라 화면으로는 안 잡힌다.
> ```
>
> **■ 코드리뷰(/code-review high)가 잡은 9건 — 넷은 이 저장소가 이미 데인 부류**
> ```
>   · ★ 구성원 문헌을 개별 발행하면 종합 카드가 통째로 지워졌다. 종합 카드 본문에 구성원의
>     PubMed 링크가 전부 있는데 중복 제거 지문이 PMID 하나뿐이었다(표 행만 고아로 남는다).
>     → data-synthesis-id 자체 지문 + 개별 발행은 종합 블록을 건너뛴다.
>   · ★ 종합을 다시 돌리면 카드가 쌓였다 — 지문이 비었고 manual 은 날짜 스윕도 건너뛴다.
>   · ★ 공통 지반의 EN 만 filter 해서 인덱스가 밀렸다("EN 2" 밑에 "한국어 1").
>     이 카드가 막으려던 오귀속이 카드 안에서 났다. 짝으로 거른다.
>   · ★ 모델이 문헌 하나를 안 적으면 그 문헌의 입장이 모든 축에서 조용히 사라졌다.
>     목록 정본을 모델 반환값이 아니라 파이프라인이 넘긴 docs 로 바꿨다.
>   나머지: 표 행 죽은 '#' → 제 카드 앵커 / 같은 제목 두 번 찍힘 / URL 묶음이 전부
>   syn:web-web 로 뭉개져 장부 충돌 → 짧은 해시 / 접힌 헤더 라벨이 카드와 다른 말 /
>   sourceText 가 종합에서 "적용했습니다" 로그만 찍고 아무 일도 안 하던 것 → 경고 후 건너뜀.
> ```
>
> **■ ★ 이 세션이 낸 실수 — /code-review 가 다른 저장소를 리뷰했다**
> ```
>   처음 /code-review high 를 그냥 돌렸더니 세션 주 작업 디렉터리(global-config)를 리뷰했다.
>   변경은 붙인 클론(trend-review)에 있었다. "리뷰 돌렸다" 는 초록이 실제로는 다른 저장소
>   초록이었다. 여러 repo 를 붙인 세션의 상시 함정이다 — 경로를 지정해서 다시 돌려야 한다:
>     /code-review high /home/user/trend-review
> ```
>
> **■ 표를 쓰지 않는다 (폰 제약)**
> ```
>   390px 에서 문헌 3건 × 문장 셀은 가로 스크롤 없이 물리적으로 안 들어간다.
>   축 단위 세로 적층 + 입장 묶음 — 같은 입장인 문헌을 한 묶음으로 묶으므로 축당 칸이
>   문헌 수가 아니라 입장 수(대개 1~3)다. 실측 폰 3,767px · 가로 스크롤 없음.
> ```
>
> **■ 검증**
> ```
>   테스트 936 → 947 전부 초록 · spec-lint 경고 0
>   신규 검사 21건. 변이 12종 전부 적색이고 각각 의도한 검사가 잡는다
>     (설계 가드 5종 + 코드리뷰 회귀 7종)
>   폰 390px·태블릿 800px 실물 렌더 PeterJ 승인 완료
> ```
>
> **■ 남은 것 (다음 세션)**
> ```
>   · 종합 카드 꼬리 링크가 죽어 있다 — 카드 최하단이 "PubMed" 로 뜨는데 종합은 자체 PMID 가
>     없어 href="#" 다. 코드리뷰가 표 행의 죽은 '#' 는 잡았는데 꼬리 링크는 놓쳤다.
>     기능 영향 없음. 다음 종합 작업 때 같이 고칠 것.
>   · 페이월 리뷰 원문 자동 보강 경로는 여전히 없다(종전 항목). 이번 종합에서도 ACC HFpEF
>     초록 본문을 못 받아 2차 출처(ACC 공식 요약)로 메웠고, 카드가 그 사실을 스스로 적었다.
> ```
>
> **[2026-08-18 — 착수 지시 6건 이행 완료 · PR #135]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.** 아래는 정답이 아니라 대조 기준선이다.
> (7) 블록의 지시 6건을 전부 했다. **7번(텔레그램)이 프롬프트에 추가로 들어왔고 그것도 했다.**
>
> **■ ★★ 가장 큰 것 — 예정리스트가 이미 발행된 논문을 1번에 놓고 있었다**
> ```
>   실측(커밋 542cfd2): queue_papers 큐 12건 ∩ selected_papers(발행장부) 51건 = 1건
>                       PMID 41188988 이 **예정리스트 맨 위**에 앉아 있었다.
>                       (PeterJ 가 텔레그램 예시로 준 바로 그 논문 = 이미 나간 것)
>   원인 둘
>     ① mergeQueueItems 가 **새로 들어오는 것만** 걸렀다. 큐에 앉은 항목은 그 뒤에
>        발행돼도 영원히 남는다. 데일리를 아무리 돌려도 안 빠진다.
>     ② ▶(on-demand)가 **논문·가이드라인에서 큐를 안 건드렸다.** 리뷰만 소진했다
>        (그 함수에 "안 그러면 며칠 뒤 데일리가 또 낸다" 는 주석까지 있었는데 리뷰 전용이었다).
>   고침: consumedIds/withoutConsumed **정본 하나**를 채우는 쪽(mergeQueueItems)과
>         그리는 쪽(_renderUpcomingFromDisk)이 같이 본다. 그리는 쪽은 발행 장부를
>         **직접** 대조한다 — ▶ 로 지금 발행하면 큐 파일은 다음 데일리까지 옛말이라
>         채우는 쪽만 고치면 그 사이 하루 동안 화면이 거짓말한다.
>         consumeTrackQueue 를 세 트랙으로 일반화 + 가이드라인 큐 소진 신설.
>   ★ 소진은 **publish 보다 앞**이다 — publisher 가 큐 파일을 커밋하기 때문이다.
> ```
>
> **■ ★★ 두 번째 — 배포본이 옛 클라이언트 코드에 멈춰 있었다 (증분 패처의 함정)**
> ```
>   생성기 CURATION_BLOCK v7  ↔  배포 3페이지 v6   (삭제 확인 문구가 옛말을 했다)
>   읽음 스크립트는 **마커가 아예 없어** 교체 경로 자체가 없었다:
>     저장소 코드엔 output/read_state.json PUT 이 있는데
>     배포 3페이지엔 grep -c read_state.json → 0 0 0
>     → 읽음을 눌러도 러너가 못 보고 다른 브라우저에도 안 넘어갔다.
>   왜 — 클라이언트 블록 교체(ensure*)가 **publish() 안에서만** 일어났다.
>        버튼 워크플로는 apply-page-render.mjs 를 태우는데 거기엔 없었다.
>   고침: ① READBTN v1 신설(마커 + 구판 스크립트 제거)
>         ② apply-page-render 가 위젯·큐레이션·보관현황·읽음을 **전부** 보장한다
>            → **버튼 한 번에 배포본이 최신 코드로 따라온다**
>         ③ test/deployedClientBlocks.test.mjs — 생성기 마커 ↔ 배포본 마커 일치 검사
>   ★★ 나도 이 함정을 세션 중에 한 번 밟았다: v8 로 올려 배포한 뒤 문구를 또 고쳤더니
>      같은 v8 이라 교체가 안 됐다. 배포본을 git checkout 하고 다시 렌더해서 풀었다.
>      **문구를 고쳤으면 버전을 올려라. 이미 그 버전으로 배포했으면 되돌리고 다시 렌더해라.**
> ```
>
> **■ 나머지 실버그 (codex Sol 감사 → 세션이 전부 실물로 재확인)**
> ```
>   · 참고자료 완료 링크가 **카드가 없는** guidelines.html 을 가리켰다 (pageSplit 은
>     reference 를 reviews.html '기타 자료' 로 보낸다) → reviews.html 로.
>   · guideline-backfill·guideline-fit 에 git pull --rebase 잔존 → reset+멱등 재적용.
>   · guideline-census 에 GHA boolean 함정(if: inputs.x == 'true') 잔존 → step output.
>   · 읽음 push 가 전송 중 변경을 잃었다(pend 면 그냥 return) → dirty 플래그.
> ```
>
> **■ 명칭 통일 (2번) — 바꾼 것과 안 바꾼 것**
> ```
>   예정리스트 · 발행리스트 · 누적리스트  (REPORT_SPEC §4-A 가 정본)
>   ★ 바꾼 것은 **사람이 읽는 문자열**뿐이다. 기계 계약은 그대로 뒀다:
>     UPCOMING:* · SECTION/GSECTION/RSECTION · ARCHIVE_START · TABLE_ROWS_* ·
>     arch-table · at-count · stat-papers-count · CURATION_BLOCK
>     이것들은 **이미 배포된 HTML 을 증분 패치할 때 다시 찾는** 이름이다. 바꾸면 조용히 깨진다.
>   "아카이브 저장 현황" → "분석 보관 현황" — 누적리스트와 헷갈리던 **별개 패널**이다.
>   버전 올림: CURATION_BLOCK v7→v8 · UPBTN v5→v6 · ARCHIVE_STATUS v2→v3 · READBTN v1 신설
> ```
>
> **■ 통계 칸 (4번)**
> ```
>   부제 "180일 · 300편 스크리닝 → 1편/일 선정" 삭제 → "매일 자동 선정 · 논문 · 가이드라인 · 리뷰"
>   분석일수 칸 제거 → 페이지당 하나: 발행 논문 / 발행 가이드라인 / 발행 리뷰 (+ 최종 업데이트)
>   ★ 칸 **구성**을 다시 그리는 정본은 pageSplit 이다. _buildPageRaw 는 페이지가 아예
>     없을 때만 도므로 거기서만 고치면 배포본은 영원히 옛 구성이다(위 함정과 같은 부류).
>   ★ spec-lint 가 "180일"·"300편" 을 **강제하고 있었다** — 지우라는 지시와 정면 충돌.
>     그 두 낱말을 정본 표기 검사에서 뺐다. 숫자 자체는 REPORT_SPEC §1 표에 남아 있다
>     (논문 트랙 수집 설정으로는 여전히 사실이다. 옛말이 된 것은 화면이 그것을
>      **선정 방식의 전부인 양** 말하는 것이었다).
> ```
>
> **■ 배포 뒤처짐 (5번)**
> ```
>   verify-pages 게이트가 daily-review **하나에만** 붙어 있었다 → curate-remove ·
>   queue-control · on-demand 에 추가.
>   ★★ concurrency 는 **검증 잡에만** 건다(group: verify-pages · cancel-in-progress).
>      28번 누르면 검증 잡도 28개지만 의미 있는 것은 마지막 하나다.
>      **본 작업 잡에는 절대 걸지 마라** — 그룹당 대기 1개라 앞 클릭이 조용히 취소된다.
>      "일관성" 을 이유로 통일하지 말 것. 방향이 반대인 것이 옳다.
>   ★ cancelled 는 failure 와 다르다 — 취소 런엔 "실패한 잡" 이 없어 rerun-failed-jobs 가
>     403/422 로 거부되고, 종전 코드는 그것을 권한 문제로 읽어 **고치는 대신 빨간 알림만**
>     보냈다. PeterJ 가 맞은 그 상황이다. → 취소는 /rerun(전체 재실행).
>   test/workflowDeployGate.test.mjs 가 위 다섯을 워크플로 텍스트 계약으로 잠근다.
> ```
>
> **■ 텔레그램 (7번 — 프롬프트에 추가로 들어온 지시)**
> ```
>   [trend-review] / 빈 줄 / 날짜 / 제목 / 저널·#PMID / 빈 줄 / 📊 링크
>   ★ **빈 줄이 규격의 일부다** (사유: 가독성). 200자 2건 분할은 **폐지** —
>     카카오(2026-08-04 폐지) 상한 유물이고, 남기면 긴 제목일 때 본문이 갈리면서
>     빈 줄 배치가 깨진다. 4096 초과 때만 제목을 잘라 구조를 지킨다.
>   ★ 채널이 메시지를 **빈 줄로** 잇는다(join('\n\n')). 빌더가 옳아도 이 한 줄이 '\n'
>     이면 진행상황이 📊 링크에 달라붙는다 — 테스트가 채널 쪽 구분자까지 검사한다.
>   md 첨부: src/utils/dailyDigest.js 신설. 그날 발행분을 **화면과 같은 축**으로 담는다
>     (요약하지 않는다 — 축을 줄이면 "첨부와 화면이 다르다" 가 되고 아무도 안 믿는다).
>   ★ **논문 0편인 날에도 보낸다** — 종전에는 진입점이 즉시 exit 0 이라 가이드라인·리뷰만
>     나간 날 알림이 통째로 없었다.
>   ★ trackDigest.js 와 sendDocument 가 **둘 다 만들어져 있는데 아무도 안 부르고 있었다**
>     (테스트만 호출). 이 저장소 최다 반복 함정. trackDigest 는 용도가 달라(트랙당 파일
>     하나·wiki 적재용) 그대로 두고 dailyDigest 를 따로 만들었다.
> ```
>
> **■ 검증** 테스트 798 → **843** 전부 초록 · spec-lint 통과 ·
> apply-page-render 무손실(표 행 26→26 · 카드 섹션 26→26) ·
> **변이 7종 전부 적색이고 각각 의도한 검사가 잡는 것까지 확인** ·
> 폰/태블릿 미리보기 4장 + MP 폰 실측(가로 스크롤 없음).
>
> **■ 진행 중이던 것 / 다음 세션이 볼 자리**
> ```
>   ① PeterJ 가 **내일부터 세 트랙 실전 관찰**. 리뷰 번역 카드 품질·coverage 정직성이
>      아직 실전 미검증이다(모양만 승인받았다).
>   ② 가이드라인 발행대기 255건 눈 감사 — 피드백이 프롬프트 조정 착수 신호.
>   ③ Google refresh token 재발급(Phase 2 아카이브 invalid_grant · 데스크탑 필요).
>   ④ 타워 갱신 — 이 세션에서 함(못 했으면 여기 "타워 미갱신" 이라고 적혀 있을 것).
> ```
>
> **[2026-08-17(7) — ★★ 다음 세션 착수 지시 6건 (PeterJ 확정 · 4종 마무리 시점)]**
>
> **■ 이것이 다음 세션의 작업 목록이다.** 위 (3)~(6) 블록은 배경이고, 실제로 할 일은 여기다.
> PeterJ 지시: **분업모드 온** · 핸드오프와 TR 대화 아카이브를 읽고 진행.
>
> ```
>  1. MP 시작부에 "이게 어떻게 돌아가나" 아주 쉬운 설명을 만든다.
>     처음 보는 사람이 직관적으로 구동 방식을 이해하게. 세 트랙 각각의
>     **선정 방식과 분석 방식**을 포함한다. (docs/master-plan.html · 같은 URL 재게시)
>
>  2. 리스트 명칭을 통일한다 — 지칭이 헷갈린다는 것이 이유다.
>       "다음 7일 예고"            → **예정리스트**
>       분석해서 올려주는 것        → **발행리스트**
>       읽음·삭제·자료화 붙은 표    → **누적리스트**
>     코드·화면·문서·커밋 메시지까지 같은 말로. grep 로 등장 위치 전수 확인.
>
>  3. 짜놓은 것을 처음부터 죽 **전체 점검**한다. Fable · codex Sol/Luna 하청을 쓴다.
>     ★ 특히 배포 페이지 버튼들이 **연결된 리스트·연동 항목과 제대로 반응하는지**
>       세 트랙 전부. (이 세션에서 버튼 실버그가 다섯 건 나왔다 — 아래 "왜" 참고)
>
>  4. 통계 칸 정리 — "180일 300편 스크리닝" 문구는 **옛말**이라 수정 또는 삭제.
>     분석일수·선정논문을 나눌 이유가 없다 → **페이지당 카운트 하나**만 남긴다.
>     "최종 업데이트" 는 그대로 둔다.
>
>  5. 배포 뒤처짐 차단 — **렌더 후 배포 확인 게이트**를 넣는다.
>     예정리스트·누적리스트에서 삭제하면 **바로 반영**되고, **여러 개 연달아 눌러도
>     로스가 없게** 한다.
>
>  6. 전체가 잘 도는지 점검을 마무리한다. PeterJ 는 **내일부터 세 트랙이 실제로
>     선정·구동되는지 지켜볼 예정**이다.
> ```
>
> **■ 3번을 왜 하는가 — 이 세션에서 버튼이 다섯 군데 거짓말을 했다**
> ```
>   · 🗑 리뷰 삭제      키 검증기·tag 화이트리스트·targets 세 자리에서 죽었다
>   · 🗑 연달아 누름     내 배치와 같은 동시성 그룹 → 앞엣것이 조용히 취소(#6 #7 cancelled)
>   · 큐 제어 push 경합  생성 HTML 에 rebase → 남의 변경을 되돌린다
>   · ▶ 승격            승격한 항목을 **맨 뒤로** 보냈다(258개 중 257·258위)
>   · 🗑 후 아카이브     분석내용·"지난 N건" 배지가 지우기 전 숫자를 말했다
>   ★ 전부 **테스트는 초록**이었다. 공통 원인은 "모듈은 옳은데 맞물리는 자리에 검사가
>     없다" 다. 3번 점검은 **버튼의 자기 설명과 실제 동작을 대조**하는 방향으로 하라.
>   ★ 검증을 함수 직접 호출로 하지 마라 — 실제 경로(워크플로 → 스크립트)를 태워야 한다.
>     이 세션이 그 실수로 "고쳤다" 고 잘못 보고했다.
> ```
>
> **■ 5번의 실측 근거**
> 삭제를 28번 연달아 누르면 GitHub Pages 가 앞 빌드를 계속 취소한다(성공 사이에 cancelled 가
> 촘촘히 끼고 마지막 성공이 2커밋 뒤처졌다). **마지막 빌드가 취소되고 그 뒤로 push 가 없으면
> 사이트가 계속 뒤처진다.** `verify-pages` 는 데일리에만 붙어 있고 버튼 경로엔 없다.
>
> **■ 2번 할 때 조심할 것**
> `CURATION_BLOCK`·`UPBTN`·`ARCHIVE_STATUS` 는 **버전 마커가 있는 클라이언트 블록**이다.
> 문구를 고쳤으면 **버전을 올려야** 증분 패치되는 배포 페이지에 반영된다(안 올리면 영원히 안 바뀐다).
> 현재 `CURATION_BLOCK v7` · `ARCHIVE_STATUS v2` · `UPBTN v5`.
>
> **[2026-08-17(6) — 삭제가 분석내용·배지까지 안 따라가던 것 · PR #131 #132]**
>
> **■ PeterJ 실측**: *"지난 논문 섹션에서 여러개 삭제했는데 논문분석 리스트에 반영이 안 됨."*
>
> **■ ★ 먼저 무엇이 **맞았는지**부터 재봤다 — 삭제 자체는 정상이었다**
> ```
>   SECTION 카드 45 → 17 · 누적 표 행 45 → 17 · 카운터 전부 일치
>   curate-remove 커밋 28건 다 올라와 있었다
> ```
> 그래서 "삭제가 안 된다" 가 아니라 **다른 두 곳이 지우기 전 숫자를 말하고 있었다.**
> 이 구분을 안 하고 삭제 경로부터 뜯으면 멀쩡한 데를 고치게 된다.
>
> **■ ① 아카이브 저장 현황에 28건 잔존 — 종전 설계가 그랬다**
> 페이지 안내문이 *"삭제한 논문도 여기엔 저장됨으로 남습니다"* 라고 적어 놨다.
> PeterJ 확정(08-17)은 그 반대다: *"삭제하면 누적리스트 및 분석내용 모두 삭제."*
> ```
>   · curate-remove 가 analysis_archive.json 에서도 빼고 블록을 다시 그린다.
>   · ★★ 재선정 방지 목록(output/selected_papers.json)은 **계속 안 건드린다** —
>     지운 논문이 다시 뽑히면 안 된다. 아카이브와 **다른 파일**인데 종전 안내문이 둘을
>     한 덩어리로 묶어 놔서 같은 것처럼 읽혔다. 문구·확인 대화상자를 정정했다.
>   · ★ 숨김 목록 **전체**를 매번 훑는다 — 이번 건만 빼면 지나간 삭제가 영구히 남는다
>     (실측: 숨김 31건 중 28건 잔존). 멱등이고 소급 정리된다.
>   · Drive·Doc 은 이미 append 된 것을 되돌릴 수 없다(누적) — 그렇게 안내한다.
> ```
>
> **■ ② "지난 논문 43건" 배지가 실제 15건**
> 이 배지는 발행 시점에 `foldPast` 가 박는 **상수**다. 삭제는 발행된 HTML 을 패치하는
> 경로라 안 건드렸다. `recountStats` 가 배지도 다시 센다 — 세는 규칙을 `foldPast` 와
> **글자 그대로 같게** 뒀다(두 곳이 다른 숫자를 말하지 않게).
>
> **■ ③ ★ 리뷰 삭제가 아직 깨져 있었다 — 어제 내 수정이 불완전했다**
> 어제 "다섯 번째이자 마지막 자리" 라고 보고한 것이 **셋 중 하나**였다.
> ```
>   sectionKey 정규식 (-r- 허용)   고쳤음
>   tag 화이트리스트                ['SECTION','GSECTION'] — RSECTION 없어 exit 1
>   targets                        ['index.html','guidelines.html'] — reviews.html 없음
> ```
> **검증을 `removeSectionFromHtml` 직접 호출로 해서 프로덕션 경로를 안 태운** 것이 원인이다.
> 세어 보면 같은 개편이 놓친 자리가 일곱이다. **"마지막" 이라고 쓰기 전에 세어라.**
>
> **■ ★★ ④ 그리고 #131 은 절반만 됐다 — 소급 정리를 실제로 돌려 보고 잡았다 (PR #132)**
> ```
>   index.html 블록 41 → 13건 · 배지 43 → 15     ← 맞다
>   analysis_archive.json 42건 그대로             ← 안 올라갔다
> ```
> 워크플로 커밋 단계가 `git add index.html output/curation_state.json` 뿐이었다.
> **증상이 고약하다**: 러너에서는 다 고쳐지고 커밋만 안 돼서 **push 가 성공으로 끝나고**,
> 블록은 프루닝된 값으로 다시 그려지니 **화면은 맞아 보이는데 다음 데일리가 되살린다.**
> 오늘 눌러 확인해도 안 잡히고 **하루 뒤에 되돌아오는** 부류다.
> **같은 자리에 두 번 빠졌다** — 08-16 에 reviews.html·트랙 큐가 add 목록에서 빠졌고
> 그때 장부에 "예외는 추적 가능하게 만들 뿐 add 를 대신하지 않는다" 라고 적었다.
> 그래서 이번에는 **검사로 잠갔다**:
> ```
>   ① 스크립트가 쓰는 모든 파일(writeFile 리터럴 + targets + 상태 파일)이 워크플로
>      git add 목록에 있는지. 대상을 4개 미만으로 찾으면 **"검사가 헛돌고 있다" 로 실패**
>      시킨다 — 검사가 조용히 무력화되는 것을 막는다.
>   ② 워크플로 tag choice 값 ↔ 스크립트 화이트리스트 일치. ③ 이 틈에서 죽었다.
> ```
>
> **■ 최종 실측 (커밋 `4ecb7e0`)**
> ```
>   analysis_archive.json  42 → 14건 · 삭제분 잔존 0 · driveState 보존
>   화면 아카이브 카운트 14건  ← JSON 과 **일치**(종전엔 13 vs 42 로 어긋났다)
>   지난 배지 15건 = 실제 지난 카드 15 · SECTION 17 = 표 행 17
>   재선정 방지 목록 51건 **유지** — 지운 논문이 다시 뽑히지 않는다
> ```
>
> **■ 배포 지연도 한몫했다** — 28번 연달아 누르면 GitHub Pages 가 앞 빌드를 계속
> 취소한다(실측: 성공 사이에 cancelled 가 촘촘히 끼었고 마지막 성공이 2커밋 뒤처졌다).
> **마지막 빌드가 취소되고 그 뒤로 push 가 없으면 사이트가 계속 뒤처진다** — 아직 안 막았다.
>
> **■ 검증** 테스트 786 → **798** 전부 초록 · spec-lint 통과 · 변이 7종 전부 적색이고
> 각각 의도한 검사가 잡는다. PR #131 #132 머지.
>
> **[2026-08-17(5) — ★ ▶ 가 승격한 항목을 맨 뒤로 보내고 있었다 · PR #129]**
>
> **■ 어떻게 걸렸나 — 누르기 전에 결과를 계산했다**
> PeterJ 가 리뷰 격리분 두 건을 ▶ 로 올리라고 했다. 누르기 전에 승격 후 순위를 계산해 보니
> ```
>   39714613 → 257/258 (rank -992.4)   40658248 → 258/258 (rank -993.8)   1위 rank 108.8
> ```
> 버튼 설명은 *"▶ 먼저 돌리기 — 다음 실행에서 이것이 나간다"* 인데 **맨 뒤로 보냈다.**
> **교훈**: 되돌릴 수 있는 조작이라도 **효과를 먼저 계산해 보면 고장이 드러난다.**
> 그냥 눌렀으면 "올렸습니다" 로 보고하고 실제로는 아무 일도 안 일어났을 것이다.
>
> **■ 원인 — 같은 날 들어간 수정 둘이 어긋났다 (둘 다 내 것이다)**
> ```
>   PR #118 (F4)  ▶ 가 status 를 queued 로 승격하게 했다. 그때는 그것이 곧 발행 순서였다
>                 (픽이 배열 머리를 봤다).
>   PR #120       정렬 정본이 llmFit.keep === false 를 **바닥(-1000)** 으로 내린다.
>                 승격은 status 만 고치고 llmFit 은 안 건드리므로 올린 항목이 그 분기에
>                 그대로 걸린다. 그리고 **정렬은 배열 위치를 안 본다.**
>   → 뒤엣것이 앞엣것의 효과를 조용히 되돌렸다. 며칠 뒤 리뷰 트랙에서 드러났다.
> ```
> **★ 테스트는 양쪽 다 초록이었다.** 승격을 검사하는 테스트는 정렬을 안 태우고, 정렬
> 테스트는 승격을 안 쓴다 — **각자 자기 절반만 봤다.** 이 저장소에서 반복되는 부류다:
> 모듈 둘이 각각 옳은데 **맞물리는 자리에 검사가 없다.**
>
> **■ 고친 것 (PR #129)**
> ```
>   · isManuallyPromoted 신설 — 판정을 **쓰는 곳(queueControl)과 읽는 곳(정렬)이 같은
>     함수**를 본다. 정렬 쪽에서 필드 이름을 따로 적으면 이름이 바뀐 날 조용히 전건
>     false 가 되고 ▶ 는 다시 "누르면 맨 뒤로 가는 버튼" 이 된다.
>   · reviewRank·guidelineRank: 수동 승격이 **무조건 1순위**(확정 ⑤-A — 수동 판단은
>     자동 필터를 우회한다). LLM 판정은 지우지 않고 증거로 남긴다.
>   · promoteInQueue: promotedAt 을 **항상** 남긴다. 종전에는 격리였을 때만 남겨서
>     이미 queued 인 항목을 ▶ 로 올려도 아무 일이 없었다(정렬이 위치를 안 보니까).
>   · papers 큐는 **종전과 완전히 같다** — status 없는 큐는 배열 순서가 곧 발행 순서라
>     표식이 의미 없다. 표식도 status 도 안 붙인다(회귀 테스트로 못 박음).
>   ★ 가이드라인에도 같은 함정이 있었다. 변이를 넣으면 가이드라인 검사까지 적색이 되는
>     것이 **두 트랙 공통이었다는 증거**다. 리뷰만 고치면 지침 버튼이 계속 거짓말을 한다.
> ```
>
> **■ 실측 결과 (커밋 `bb38635` · `fb3bfd5`)**
> ```
>   승격분 rank -992.4 → +1007.6
>   실물 리뷰 큐 상위: 1 자원제한 패혈증 [수동승격] · 2 소스컨트롤 [수동승격] ·
>                      3 Lancet Sepsis · 4 NEJM Sepsis and Septic Shock · 5 ICM ARDS/ECMO
>   배포 reviews.html 예고 블록도 같은 순서 — 화면과 픽이 같은 것을 본다.
> ```
> PeterJ 판단: **임계값을 6→5로 내리지 않는다.** 내리면 패혈증 종설과 함께 CRT·건선까지
> 올라온다. 살릴 것은 개별 ▶ 로 올린다.
>
> **■ 검증** 테스트 782 → **786** 전부 초록 · spec-lint 통과 · 변이 3종 전부 적색이고
> 각각 의도한 검사가 잡는다. PR #129 머지.
>
> **[2026-08-17(4) — 트랙3 리뷰에 LLM 셀렉 · PR #127]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.**
>
> **■ PeterJ 확정 3-1**: *"트랙3 397건에도 LLM 셀렉 걸어라."* 그런데 가이드라인 셀렉을
> 그대로 돌리면 **안 됐다.** 실물을 열어 보고 나온 두 가지가 이 PR 의 전부다.
>
> **■ ★★ ① 프롬프트를 재사용하면 397건이 전건 격리된다**
> `buildFitPrompt` 의 점수 기준 마지막 줄이 이렇다.
> ```
>   - 0~2  이 독자와 무관 (… 지침이 아니라 지침을 연구한 논문, 학회 행정문서)
> ```
> 리뷰 아티클은 **정의상 전부** "지침이 아닌 것" 이다. 그대로 돌리면 LLM 이 성실하게
> 0~2 점을 주고 397건이 다 내려간다. 더 나쁜 것은 그 결과가 **"셀렉이 잘 걸렀다" 로
> 읽힌다**는 점이다 — 숫자만 보면 정상 작동처럼 보인다.
> → 리뷰 전용 프롬프트(`src/utils/reviewFit.js`)를 만들고 *"지침이 아님은 감점 사유가
>   아니다"* 를 명시했다. 그 문구를 지우면 적색이 되는 계약 테스트를 붙였다.
> **교훈**: 셀렉 기준은 트랙마다 다르다. 같은 문서가 한쪽에서 10점이고 다른 쪽에서
> 2점인 것이 정상이다. 기계(배치·전역 index·격리 정책)는 공유하고 **기준만 가른다.**
>
> **■ ★★ ② 판정을 써도 아무도 안 본다 — 리뷰 큐엔 status 도 정렬도 없었다**
> ```
>   실측 큐 항목: {pmid, title, journal, score, topic, lowConfidence, addedAt}
>                 → 397건 전원 status 없음
>   _stageReview: const [picked, ...remaining] = state.queue   ← 배열 머리를 그냥 집었다
>   예고 렌더:     리뷰는 큐를 통째로 그렸다
> ```
> 이 상태로 셀렉만 붙이면 `status:'needsReview'` 가 써지는데 **픽도 화면도 그 필드를
> 쳐다보지 않아** 격리한 것이 그대로 발행된다. 커밋까지 성공하고 동작은 아무것도 안
> 바뀐다 — 이 저장소가 여러 번 데인 "모듈은 옳은데 안 불린다" 부류다.
> → `src/utils/reviewRank.js` 신설(정렬·필터 정본). 픽과 예고가 **같은 함수**를 쓴다.
>
> **■ ★ 지킨 것 세 개 (다음 세션이 되돌리기 쉬운 자리다)**
> ```
>   ① status 없음 = 발행 가능. 없음을 격리로 읽으면 필터를 붙이는 순간 미판정
>      397건이 통째로 화면에서 사라진다. 격리는 명시적 needsReview 뿐이다.
>   ② 격리는 rejected 가 아니다. mergeQueueItems 가 rejected 를 영구 배제로 다뤄
>      한 번 잘못 자르면 다시는 안 들어온다(PR #120 에서 같은 결정을 했다). ▶ 로 되살린다.
>   ③ 데일리 코어 무영향 — 리뷰 큐를 채우는 것은 데일리가 아니라 build-review-queue.mjs
>      다(실측: 데일리는 _saveTrack1Queue 로 논문 큐만 채운다). 그래서 리뷰에는
>      **데일리 증분 판정을 안 붙였다.** 데일리에 LLM 호출이 늘지 않는다.
>   · promoteInQueue(▶)는 손대지 않았다 — 이미 status 를 일반적으로 승격해서, 리뷰가
>     status 를 갖는 순간 자동으로 옳게 동작한다(PR #118 F4 교훈).
> ```
>
> **■ 벌크 실행 결과 (실측 · run 32006139254 · 커밋 `80c1ca2`)**
> ```
>   397건 전량 판정 · 채택 256 · 격리 141 · 발행 가능 256
>   점수 분포: 10점 2 · 9점 71 · 8점 60 · 7점 55 · 6점 68 · 5점 27 · 4점 27
>              3점 37 · 2점 37 · 1점 12 · 0점 1
>   ★ 이 **분포 자체가 ① 이 고쳐졌다는 증거다.** 프롬프트를 재사용했다면 0~2 점에
>     397건이 몰렸을 것이다. 실제로는 6점 이상이 256건으로 퍼졌다.
>   상위 7 = Lancet Sepsis · NEJM Sepsis and Septic Shock · ICM ARDS/ECMO 60년 ·
>            CCM ECPR 후 심정지 관리 · ICM PEEP·폐동원 · ICM 병원전 기도 · ICM Prone
>   → 배포된 reviews.html 예고 블록의 저널 순서가 이 랭크와 **정확히 일치**함을 확인했다
>     (Lancet → NEJM → ICM → CCM → ICM×3). 화면과 픽이 같은 것을 본다.
> ```
>
> **■ PeterJ 가 볼 자리 — 경계선 5점 격리분**
> 낮은 점수 격리는 명확하다(피부과·안과·종양·정책문서·연구방법론). **5점 27건이 회색지대**다.
> 그중 다음 둘은 되살릴 값이 있어 보인다 — ▶ 로 올리면 된다:
> ```
>   ICM  Management of adult sepsis in resource-limited settings (자원제한 패혈증)
>   ICM  Source control in bloodstream infections in patients with sepsis (소스컨트롤)
> ```
> 패혈증 계열인데 "자원제한·연구표준화 색채" 로 깎였다. 임계값(6점)을 5로 내리는 것보다
> 개별 ▶ 가 낫다 — 임계값을 내리면 CRT·건선 같은 것도 같이 올라온다.
>
> **■ 검증** 테스트 767 → **782** 전부 초록 · spec-lint 통과 · 변이 4종 전부 적색이고
> **각각 의도한 검사가 잡는 것까지** 확인했다(적색 여부로 끝내지 않는다).
> 특히 픽 필터를 벗기면 "미판정 회귀" 검사까지 적색이 되는데, 그것이 그 회귀 검사가
> 배열 순서가 아니라 **정렬을 실제로 태우고 있다는 증거**다.
> 계획서 `docs/superpowers/plans/2026-08-17-review-llm-selection.md`.
>
> **[2026-08-17(3) — 2년 풀 실적립 · 3트랙 분석 분리 · 버튼 실버그 3건 · PR #122~#125 머지]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.** 아래는 정답이 아니라 대조 기준선이다.
>
> **■ ★ 2년 풀이 실제로 찼다 — 이제 숫자가 추정이 아니다** (실측 2026-08-17, 상태 파일)
> ```
>   가이드라인 큐 672건 = queued 255 + needsReview 417   (672건 전부 LLM 판정 완료)
>     · needsReview 417 은 "버린 것"이 아니라 격리다 — 예고 리스트 ▶ 로 언제든 복구된다
>     · 상위 12건 전부 llmFit.score = 10  → 데일리가 며칠은 최상위만 먹는다
>   리뷰(트랙3) 큐 397건 · 논문 큐 12건
>   → 데일리 월 30편 소비 기준으로 가이드라인만 8개월분. **더 이상 "리스트가 비었다" 가 아니다.**
> ```
> 2년 백필(monthly:24 · apply)과 LLM 셀렉 벌크(limit 700)를 **내가 Actions 로 돌렸다**
> (PeterJ 지시: *"2년치 풀 내가 안만들고 니가 하청주든 해서"*). 세션 컨테이너에서는
> PubMed 프록시가 403 이라 Actions 만이 통로다 — 다음에도 같은 두 워크플로를 쓰면 된다.
>
> **■ ★★ 3트랙 분석 분리 (PR #125 — PeterJ 확정)**
> *"1.2.3.트랙은 분석이 다 달라야함"*
> ```
>   ① 논문      종전 PICO 그대로. 한 줄도 안 건드렸다.
>   ② 가이드라인 실무 관점 요약 + 이전 버전과의 차이. **실물 프롬프트를 열어 보니 이미
>                summary + keyChanges 로 그걸 하고 있었다** → 변경 없음(중복 구현 금지).
>   ③ 리뷰      요약이 아니라 **번역**. 새 mode:'review' 신설.
>      · _reviewTool() 스키마에서 summary·keyChanges·sourceNote 를 **뺐다**.
>        도구에 칸이 없으면 모델이 요약으로 흐를 자리가 없다 — 프롬프트 문구보다 강한 장치다.
>      · 대신 원문 절 순서를 따르는 sections[{heading_ko, body_ko}] 와
>        coverage(full-text / web-augmented / abstract-only).
>      · 초록만으로 때우기 전에 WebSearch·WebFetch 로 원문을 찾게 하고, 못 찾았으면
>        coverage 에 **정직하게** 적게 한다(카드에 안내문으로 노출 → 거짓말이 화면에 남는다).
>      · sections 없고 summary 만 있는 **구판 카드는 종전대로** '핵심 내용' 으로 그린다.
>        과거 발행물을 새 렌더로 깨지 않기 위한 분기다.
> ```
>
> **■ 버튼 실버그 3건 (PR #122 #123 #124 — 전부 PeterJ 폰 실측에서 나왔다)**
> ```
>   ① 🗑 리뷰 삭제가 안 먹었다 — scripts/curate-remove.mjs 의 키 검증기가 `-m-` 만 알고
>      `-r-` 를 몰라 "✖ 잘못된 sectionKey" 로 죽었다. **RSECTION 이 빠진 다섯 번째 자리.**
>      08-16 3트랙 개편이 남긴 같은 부류의 마지막 구멍이다(앞 넷은 PR #121).
>      → 실물 reviews.html 로 확인: 카드와 표 행이 같이 사라졌다(62,632 → 60,244 bytes),
>        GSECTION 은 안 건드림.
>   ② 🗑 를 연달아 누르면 앞엣것이 **조용히 취소**됐다 — 내 배치 워크플로가 버튼과
>      같은 concurrency 그룹(queue-control)을 썼다. GitHub 은 **그룹당 대기 1개만** 유지한다.
>      실측: run #6 cancelled · #7 cancelled · #8 pending, 화면은 무반응.
>      → 배치는 guideline-batch 로 분리, 버튼은 그룹을 **아예 안 쓴다**.
>   ③ queue-control 이 push 경합 때 `git pull --rebase` 를 했다 — 생성 HTML 은 **항상**
>      충돌하고(실측 로그: CONFLICT in index.html), 설령 성공해도 낡은 파일을 다시 써서
>      그 사이 들어온 다른 실행의 변경을 **되돌린다**. → 최신 main 으로 reset 한 뒤
>      변형(멱등)을 다시 적용하는 루프로 교체.
> ```
>
> **■ 소아 전용 자동 배제 (PeterJ A-1 확정)**
> `src/utils/guidelinePediatric.js` — 규칙으로 소아 전용만 자동 배제하고, 인접 전문과는
> LLM 판단에 맡긴다. ADULT 정규식에 `basic life support|acls|bls` 를 넣어 성인 소생술
> 지침이 소아로 오인되지 않게 했다. manualApproved 는 무조건 통과.
>
> **■ GHA 함정 — boolean 입력은 API 로 오면 문자열이다 (두 번 데였다)**
> ```
>   if: inputs.apply        → "false" 도 truthy (비어 있지 않은 문자열)
>   if: !inputs.dry_run     → "false" 에 대해 거짓 → **진짜 실행의 커밋이 조용히 건너뛰어진다**
>   if: inputs.x == 'true'  → 이것도 틀렸다. GHA 는 타입이 다르면 숫자로 캐스팅한다
>                             (true→1, 'true'→NaN)
>   ★ 해법: 셸에서 판정해 step output 하나로 내보내고 뒤 단계가 그것만 본다.
> ```
>
> **■ 실전에서 아직 안 본 것**
> ```
>   ① 리뷰 번역 카드의 실제 품질   /preview 로 모양은 승인받았다. **실제 LLM 번역은 다음 데일리가 첫 실전.**
>      특히 coverage 가 정직하게 붙는지 · web-augmented 가 실제로 웹을 뒤지는지.
>   ② LLM 셀렉 판정 품질           queued 255 를 PeterJ 가 눈으로 감사한 뒤 프롬프트를 조정한다
>                                  (*"한동안 좀 돌려보고 피드벡 할게"* — 그 피드백이 착수 신호).
>   ③ 트랙3 397건에 LLM 셀렉        아직 안 걸었다. 걸지 여부는 PeterJ 미결.
> ```
>
> **■ 검증** 테스트 728 → **767** 전부 초록 · spec-lint 통과 · 변이 주입 전부 적색.
> PR #122 #123 #124 #125 머지.
> **PR CI 는 이 세션 토큰 push 에 안 붙는다**(GitHub 재귀 방지 — `ci.yml` 주석에 기록됨).
> close/reopen 도 안 통했다. 로컬 증거로 판단하고 main push CI 가 재확인한다.
> **배포 URL 은 못 봤다**(프록시가 github.io 를 막는다).
>
> **[2026-08-17(2) — 가이드라인 셀렉션 전면 개편 · PR #118~#121 머지]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.** 아래는 정답이 아니라 대조 기준선이다.
>
> **■ PeterJ 지적에서 출발했다** — *"리스트 비어있는게 이상"* · *"2년치 미리 풀링하고
> 신규는 add on 하기로 하지 않았나"* · *"셀렉은 LLM 으로"* · *"cardiac 같은 넓은 건 빼고
> 질병명 위주로"*. 문서를 대조하니 **PeterJ 기억이 맞았고, 코드에 안 붙어 있었다** —
> 08-16 세션이 결정 6건을 물어놓고 답을 못 받은 채 끝나서(이 파일 285줄) 옛 상태에 멈춰 있었다.
>
> **■ ★★ 왜 예고 리스트가 비었나 — 화면이 아니라 판정이었다 (PR #118)**
> 큐 5건이 **전부 needsReview** 였다. 분류기는 양성 증거 4축 중 2축을 요구하는데
> **3축이 구조적으로 항상 false** 였다.
> ```
>   F1 esummary 필드명 오타 — pubtypelist 를 읽었다. NCBI 의 실제 필드는 pubtype.
>      논문 트랙(DataCollectorAgent)·census 는 처음부터 pubtype 을 쓴다. 여기만 틀렸다.
>      실측: PubMed 수집분 8건 전부 publicationTypes: []
>      → format·official 두 축 사망. PT 쿼리로 찾은 42522393 이 documentType:null 이었다.
>      ★ 테스트가 초록이던 이유: 픽스처가 같은 오타를 박아 뒀다.
>   F2 초록을 한 번도 안 받았다 — esummary 는 초록을 안 주는데 normative 축은 거기서만 읽는다.
>      실측: 상태 파일 15건 전부 abstract 없음. → efetch 보강(enrichCandidates) 신설.
>   F3 분류기가 PT 근거를 publicationTypes 하나에만 걸었다 → discoveredBy 도 근거로.
>   F4 ▶(promote)가 배열 순서만 바꿨다. 발행 픽·예고가 status==='queued' 만 보므로
>      needsReview 는 올려도 아무 일이 없었다 — 소진 통로 0. 이제 승격까지 한다.
>   F5 guideline-backfill.yml 이 --apply 없이 돌아 리포트만 내고 큐는 안 채웠다.
> ```
>
> **■ 풀링 구조 교체 (PR #119)**
> ```
>   · config/guideline-topics.json(질병명 188개·11그룹)이 08-15 에 만들어졌는데
>     2026-08-17 까지 **아무도 안 읽었다** — 참조처가 자기 테스트뿐이었다.
>     그동안 수집은 EM/CCM MeSH 8개에만 묶여 30일 창에서 8건. 데일리는 월 30편을 먹는다.
>   · 주제 그룹마다 쿼리 하나(src/utils/guidelineTopics.js). ⓐ 한 줄로 이으면 10KB 넘어
>     esearch GET 이 깨진다 ⓑ 어느 주제가 마르는지 manifest.topics 에 그룹별로 남는다.
>   · 넓은 단어는 안 들어간다(PeterJ 확정 08-15) — cardiac·trauma·airway·transfusion·
>     triage·resuscitation·emergency medicine 전부 없음. 테스트가 재도입을 막는다.
>   · 2년 캐치업 = monthly:24 (30일씩 24조각). 한 창(730-0)으로 캐면 retmax 가 최신만
>     주고 나머지가 조용히 사라진다. 워크플로 기본값을 monthly:24 로.
>   · 데일리는 그대로 최근 30일 = add-on. mergeCandidates 가 대조하므로 신규만 얹힌다.
>   · 레이트리밋 — 쿼리가 2개→13개. 동시 2개 제한 + 백오프. census 가 429 로 죽은 적 있다.
> ```
>
> **■ LLM 셀렉 (PR #120)**
> ```
>   src/utils/guidelineFit.js        순수 함수 (배치·프롬프트·판정). 네트워크 없음
>   src/agents/GuidelineFitAgent.js  LLM 호출(배치 20). 실패는 전부 소프트
>   src/utils/guidelineRank.js       ★ 정렬 정본 — 발행 픽과 예고 리스트가 같이 쓴다
>   rank = keep:false ? -1000+priority : 적합도×10 + priority
>   · 안 맞는 것을 rejected 로 안 내린다 — mergeCandidates 가 영구 배제로 다뤄서 한 번
>     잘못 자르면 다시는 안 들어온다. needsReview 로 격리(▶ 로 복구 가능).
>   · 빠뜨린 판정은 0점이 아니라 무판정. 프롬프트는 "애매하면 keep:true".
>   · ★ 붙이자마자 계약 테스트가 적색 — PeterJ 수동 승인 문서를 LLM 이 격리해서 그날
>     발행이 통째로 사라졌다(확정 ⑤-A 위반). unscoredItems 가 manualApproved 를 제외한다.
>   · 데일리 하루 40건 상한. 끄는 스위치 ENABLE_GUIDELINE_LLM_FIT=false.
> ```
>
> **■ 대시보드 버튼 두 개 (PR #121 — PeterJ 실측)**
> 둘 다 원인이 같다: **RSECTION(리뷰)이 08-16 3트랙 개편에서 생겼는데 큐레이션·
> on-demand 경로가 그때 안 따라왔다.**
> ```
>   🗑 누적 리스트 — "섹션 키를 찾지 못했습니다". 네 군데가 동시에 SECTION|GSECTION 만 알았다:
>      클라이언트 정규식 · removeSectionFromHtml 화이트리스트 · parseHiddenKey ·
>      curate-remove.yml 의 tag choice 목록. → SECTION_TAGS 한 곳으로 모았다.
>   ▶ 예고 리스트 — 리뷰만 큐 순서 변경으로 폴백했다(on-demand 에 review kind 가 없어서).
>      kind=review 신설: 데일리 _analyzeReview 와 같은 부품으로 카드 → reviews.html 에
>      RSECTION → **큐에서 소진**(안 그러면 며칠 뒤 데일리가 같은 것을 또 낸다).
>   ★ 계약 검사가 입력 **이름**만 봤다. choice **값**이 목록에 없으면 422 로 튕긴다 —
>     이번 두 건이 정확히 그 자리다. 값까지 대조하는 검사를 붙였다.
> ```
>
> **■ ★★ PeterJ 가 손대야 할 것 — Actions 2단계 (이걸 해야 리스트가 찬다)**
> 2년 풀은 세션 컨테이너에서 못 만든다(PubMed 프록시 403). 순서대로 돌려야 한다.
> ```
>   ① Actions → "Guideline backfill (캐치업 백로그)" → Run workflow
>      windows = monthly:24 (기본) · apply 체크박스 ON → 실행
>   ② 끝난 뒤 Actions → "Guideline fit (LLM 셀렉 벌크)" → Run workflow → limit 400 → 실행
> ```
> 그 다음 **queued 목록을 눈으로 감사**해야 한다 — 프롬프트 문구는 그 결과를 보고 조정한다.
>
> **■ 아직 실전에서 안 돌아본 것** (이 저장소가 아홉 번 넘게 데인 자리다)
> ```
>   ① 주제축 실회수량      2년 백필을 돌려야 나온다
>   ② LLM 셀렉 판정 품질   벌크를 돌린 뒤 queued 전수 감사
>   ③ ▶ 리뷰 지금 실행     다음에 누를 때가 첫 실전
>   ④ 🗑 리뷰 누적 삭제     같음
> ```
>
> **■ ★ 정정 — 추정치를 실측처럼 쓰지 마라**
> 시장조사의 **연 2,888편은 interests.json(넓은 단어 포함 논문축)으로 잰 값**이다.
> 지금 거는 것은 **좁은 질병명 축**이라 그보다 적다. 세션 중에 한 번 그 숫자를
> 새 축의 예상치처럼 말했다가 정정했다. 실제 값은 ①로만 나온다.
>
> **■ 이 세션이 낸 실수 셋 — 전부 테스트가 잡았다**
> ```
>   ① 주석에 백틱으로 `kind` 라고 적었더니 _upcomingScript 의 템플릿 리터럴이 끊겨
>      파일 전체가 SyntaxError. 어제 TOWER_TONE 사고와 **같은 부류**다.
>   ② 정렬 변이 둘이 초록으로 통과했다 — 테스트가 한쪽을 격리시켜 놔서
>      status==='queued' 필터만으로 답이 갈리고 정렬은 관여하지 않았다.
>   ③ 지역 필터 변이도 초록이었다 — 걸러졌어야 할 것이 needsReview 로 떨어지면
>      감사 목록 두 곳 어디에도 안 나타난다. 판정에 들어간 후보 수를 직접 못 박아 고쳤다.
>   ★ 그리고 **테스트가 실제 LLM 을 때리고 있었다**(계약 테스트 18초). 판정 결과에 따라
>     초록·적색이 갈리는 비결정 테스트다. ENABLE_GUIDELINE_LLM_FIT=false 로 막았다.
> ```
> **교훈**: 변이 주입을 "적색이 나오나" 로 끝내지 마라. **왜 적색인가**까지 봐야
> 필터가 답을 가른 것인지 검사가 답을 가른 것인지 갈린다.
>
> **■ 검증** 테스트 672 → **728** 전부 초록 · spec-lint 통과 · 변이 주입 14건 전부 적색.
> PR #118 #119 #120 #121 머지. **배포 URL 자체는 못 봤다**(프록시가 github.io 를 막는다).

> **[2026-08-17 — 요구 6건 반영 · 첫 실전 관찰 · ★ 가이드라인 백로그가 비어 있다]**
>
> **■ 이 문서를 믿지 말고 실물부터 재확인할 것.** 아래는 정답이 아니라 대조 기준선이다.
>
> **■ 첫 실전(08-17 06:30) 결과 — 확인 완료**
> ```
>   예고 리스트   세 페이지 각각 자기 트랙 것만 떠 있다
>   queue_papers  살아남았다 — 13건 · updatedAt 2026-08-17 (커밋 목록 수정이 바로 먹혔다)
>   리뷰 arm      발행됐다 — lastRun 2026-08-17 published · 저수지 398건
>   카드 접힘     세 페이지 펼쳐진 카드 0
> ```
>
> **■ PeterJ 요구 6건 반영 (PR #112 · #114)**
> ```
>   ① 🗑 이 화면에 반영 안 되던 것 — 큐 제어가 페이지를 다시 그리지 않았다.
>      실측: 42087034 를 뺐는데 예고에는 그대로 남았다. 버튼은 "1~2분 뒤 반영" 이라 말했다.
>      → queue-control.yml 이 apply-page-render 를 태우고 세 페이지를 함께 커밋한다.
>   ② 예고 리스트도 기본 접힘 (접힌 상태에서 건수는 보인다)
>   ③ 제목이 안 보이던 것 — 기기가 다크모드면 .up-day{color:#d1d5db} 를 제목이 상속해
>      흰 바탕에 흰 글씨가 됐다(실측 rgb(209,213,219)). 이 페이지는 배경이 라이트 전용이라
>      다크 규칙 자체가 성립하지 않는다. 66자 절단도 제거 — 잘리면 삭제 판단이 불가능하다.
>   ④ 지난 카드를 한 겹 더 접었다 (오늘 것만 밖에)
>   ⑤ '검토함' 제거 — 분류기 진단이지 읽고 무엇을 하는 화면이 아니었다. 렌더러가 두 곳이라
>      둘 다 걷고, 배포본에 남은 블록을 지우는 경로는 남겼다(유령 방지). 데이터는 그대로다.
>   ⑥ 리뷰 아티클 — 둘 다 틀려 있었다.
>      ⓐ 첫 발행분이 Delphi 합의문이었다. Review[Publication Type] 은 합의문·지침까지
>        포함하는데 쿼리는 SR·메타만 뺐다. 제목 판정으로 저수지 398 중 64건을 거른다.
>        이미 쌓인 것은 소비하며 정화한다. 'standard of care' 는 일부러 뺐다(진짜 종설이 걸렸다).
>      ⓑ 카드에 분석이 없었다 → 분석기 reference 모드로 번역·정리 카드를 만든다.
>        NEJM syncope 카드와 같은 경로다. 원문 없으면 초록, 분석 실패해도 얇은 카드로.
> ```
>
> **■ ★★ 다음 세션 첫 과제 — 가이드라인 캐치업 백로그가 비어 있다**
> PeterJ 지적: *"시작시점인데 엄청 예비 많아야하는거 아냐?"* — 맞다. 실측하니 이렇다.
> ```
>   가이드라인 상태 전체   queue 5(전부 needsReview) · published 7 · rejected 3 = 15건
>   발견 경로             pubmed-pt 2 · pubmed-title 8 · 레거시 7 · org-source 0
>   발견일                2026-08-15, 08-16 — 이틀치뿐
>   데일리 수집 창         오늘 기준 최근 30일
> ```
> 원인 둘: ①데일리는 **최근 30일만** 본다(흐름이지 캐치업이 아니다) ②백로그를 채우라고
> 만든 **`guideline-backfill.yml` 이 한 번도 안 돌았다**(기본 창 `60-30,150-120,240-210`,
> 상태 파일에 그 경로 항목 0건). **학회 사이트 수집(org-source)도 0건이다.**
> 시장조사 실측은 관심주제 지침 **연 2,888편** 규모였다 — 물건이 없는 게 아니라 캔 적이 없다.
> ★ 백필은 LLM 을 안 쓰고 PubMed 만 때린다. 다만 **이 컨테이너는 PubMed 가 프록시에 막혀
>   있어** Actions 워크플로로 디스패치해야 한다.
>
> **■ 내일 06:30 에 가이드라인은 안 나간다**
> PeterJ 가 `ENABLE_GUIDELINE_AUTOPUBLISH=true` 를 켰지만, 그 뒤 🗑 로 지운 것이
> **큐에서 유일하게 발행 가능하던 1건**이었다(나머지 5는 needsReview). 나갈 것이 없다.
> `needsReview` 5건을 어떻게 할지(판정 기준을 풀지 · 수동 승격 통로를 만들지)가 논점이다.
>
> **■ 아직 실전에서 안 돌아본 것 셋** (이 저장소가 아홉 번 넘게 데인 자리다)
> ```
>   ① 리뷰 번역 카드        내일 06:30 이 첫 실전
>   ② 🗑 → 페이지 재렌더     다음에 버튼 누를 때가 첫 실전
>   ③ 리뷰 필터 정화        내일 저수지에서 합의문을 걸러내며 도는 첫 실전
> ```
>
> **■ 이 세션이 낸 실수 둘 — 같은 부류가 재발한다**
> ```
>   ① 스타일 상수에 백틱을 하나 넣어 TOWER_TONE 이 문자열이 아니라 boolean true 가 됐다.
>      node --check 는 **문법상 유효해서 통과**한다. ensureTowerTone 이 구버전을 걷어낸 뒤
>      빈 것을 끼워 넣어 배포 세 페이지의 스타일이 통째로 날아갔다(각 -6.5KB).
>      → 상수가 문자열인지 · </style> 로 닫히는지 · 얹었을 때 페이지가 줄지 않는지를 검사한다.
>   ② 무손실 테스트 픽스처로 **배포 실물**을 썼다. 이미 기능이 적용된 결과물이라
>      기능을 없애는 변이도 초록이었다(코드리뷰가 잡았다). 합성 픽스처로 바꿨다.
> ```
> **교훈**: 배포 실물을 픽스처로 쓰지 마라. `if (!html.includes(X)) continue` 가드는
> **기능이 죽으면 검사도 같이 죽는다.**
>
> **■ 검증** 테스트 590 → **672** 전부 초록 · spec-lint 통과 · 무손실 53→53 ·
> 폰 390px 라이트·다크 실렌더 확인. PR #110 #111 #112 #114 머지.
> **배포 URL 자체는 못 봤다** — 이 컨테이너 프록시가 github.io 를 막는다.

> **[2026-08-16(4) — 3페이지 재구성 완료 · 배포 반영까지]**
>
> **■ 무엇이 끝났나** — PeterJ 요구 5건을 **한 설계로** 붙이고 배포 페이지까지 반영했다.
> ```
>   ① 2 → 3 페이지   index=논문 / guidelines=가이드라인 / reviews=리뷰·기타
>                    누적 표도 페이지별. 기타가 가이드라인 → 리뷰 쪽으로 옮겨갔다
>   ② 예고 페이지별   마커를 <!-- UPCOMING:<track> --> 로 갈라 각 페이지 맨 위에.
>                    🗑 ▶ 토글 · 순차진행 · 갈아엎기 버튼이 그 안에 같이 간다
>   ③ 전부 접힘       오늘 논문 포함. 배포본에 남은 옛 open 도 collapseAllCards 가 걷는다
>   ④ 주기 4-A       세 트랙 전부 매일. 정본은 src/utils/trackCadence.js **하나**
>                    + 순차진행 토글(논문→가이드라인→리뷰 하루 한 트랙, 기본 꺼짐)
>   ⑤ 누적표 접힘     <details class="arch-fold">, 기본 접힘
> ```
>
> **■ ★★ PeterJ 가 손대야 할 것 하나 — 이것만 남았다**
> **`ENABLE_GUIDELINE_AUTOPUBLISH` 저장소 변수를 `true` 로.** 지금 값이 없어서
> 가이드라인이 **관찰 모드**로만 돈다(`lastRun.outcome: "observe-only"`).
> ④를 "매일 누적" 으로 만들어도 이 스위치가 꺼져 있으면 한 편도 안 나간다.
> 세션이 대신 켤 수 없다(Actions 변수는 저장소 설정이다).
> 경로: 저장소 → Settings → Secrets and variables → Actions → **Variables** 탭 →
> New repository variable → Name `ENABLE_GUIDELINE_AUTOPUBLISH` · Value `true`.
>
> **■ ★ 핸드오프 오진 정정 — "큐가 6건뿐인 건 주 1회 게이트 때문" 은 틀렸다**
> `_guidelineDue()` / `guidelineIntervalDays` 는 **정의만 되고 아무도 부르지 않는 죽은
> 코드**였다(확정 ④-D/G7 이 게이트를 이미 없앴고, 잔해만 남아 있었다). 문서와 죽은 코드가
> 서로를 보증해서 다음 세션(나)을 오진하게 만들었다. **지웠다.** 실제로 막던 것은 관찰 모드다.
> 큐 6건의 실제 구성도 재보니 `queued 1 · needsReview 5` 였다 — 공급도 게이트도 아닌
> **판정** 문제다. needsReview 5건을 어떻게 처리할지가 다음 논점이다.
>
> **■ 실물에서 잡은 결함 5건 — 전부 테스트가 초록이던 것들**
> ```
>   B1 배포된 index.html 의 예고 버튼이 전부 죽어 있었다
>      var OWNER='undefined' 가 라이브 스크립트에 구워져 있었다(owner=env, 러너 밖 렌더).
>      ONDEMAND·CURATION 이 멀쩡했던 건 실력이 아니라 그 둘은 "없을 때만 삽입" 이라서다
>   B2 예고가 거짓말을 했다 — 가이드라인을 cadence:'daily' 로 그리는데 게이트는 7일
>   B3 리뷰 트랙이 화면에 아예 안 나왔다 — _stageReview 가 큐만 옮기고 카드·행을 안 만듦
>   B4 예고 버튼 3종이 받는 쪽 계약과 어긋남 — ▶ {pmid,mode} vs {target,kind}(422),
>      🗑 은 발행섹션 삭제를 호출, ♻ 의 '__UPCOMING_RESET__' 는 아는 코드가 없었다
>   B5 _renderUpcomingFromDisk 가 ko-KR 시각을 slice(0,10) — 내일 데일리부터 예고가
>      통째로 빠질 상태였다(예외를 publish 의 try/catch 가 삼킨다)
> ```
> **처방으로 박은 못**: 워크플로 입력 계약 검사(버튼이 보내는 키를 실제 yml 의 inputs 와
> 대조), 식별자 'undefined' 금지 검사, ko-KR 시각 회귀, 큐 제어 CLI 실경로 호출 검사.
> **전부 변이를 주입해 적색이 되는 것까지 확인했다.**
>
> **■ 3분할 무손실 — 마커의 집이 또 바뀌었다**
> `test/threePageNoLoss.test.mjs` 가 **마커마다 "어느 페이지에 있어야 하는가"** 를 못 박고
> **기준값이 0이면 스스로 실패**한다. 대리 마커는 금지했다 — `직접 지정` 처럼 정적 UI 에도
> 있는 문구는 기준값이 0이 아니어도 **보호 대상이 전멸하는 변이를 못 잡는다**(코덱스 감사 발견).
> 배포 반영 실측: **표 행 51→51 · 카드 섹션 51→51 · reviews.html 신규 57KB**.
>
> **■ ★ 병렬 코드리뷰가 잡은 것 — 이 세션의 절반이 여기 들어갔다**
> 코덱스와 Claude 리뷰어를 동시에 돌렸고 **둘 다 실버그를 냈다**(중복 없음).
> ```
>   치명 publish 의 표 행 삽입이 함수형 replacer 가 아니었다 — LLM 제목에 $& 나 $` 가
>        있으면 본문이 통째로 복제된다. 실측 575KB → 1.37MB. esc() 는 못 막는다
>   치명 reviews.html 과 트랙 큐가 git add 목록에 없었다 — push 는 성공으로 끝난다
>   높음 트랙 토글이 아무것도 안 막았다 — mode 를 읽는 게이트가 리뷰 하나뿐
>   높음 가이드라인에서 뺀 항목이 다음 수집에 되살아났다(rejected 미대조)
>   높음 리뷰의 ▶ 가 index.html 에 '직접 지정 논문' 으로 발행했다
>   높음 ★ 내가 만든 무손실 검사 3건이 헛돌았다 — 배포 실물을 픽스처로 써서
>        기능을 통째로 없애는 변이도 초록이었다("기능이 죽으면 검사도 사라진다")
>   높음 순차진행 시 3일 중 2일은 전량 분석 후 버리는데 텔레그램은 그대로 나갔다
> ```
> **배운 것 (다음 세션도 지킬 것)**: ①**배포 실물을 픽스처로 쓰면 안 된다** — 이미 기능이
> 적용된 결과물이라 기능을 없애도 통과한다. 합성 픽스처로 "아직 적용 안 된 입력" 을
> 일부러 만들어라. ②`if (!html.includes(X)) continue` 가드는 **기능이 죽으면 검사도 같이
> 죽는다.** ③**두 리뷰어를 같이 돌려라** — 겹치는 발견이 하나도 없었다.
>
> **■ 다음 세션이 볼 것 (08-17 06:30 이 여전히 첫 실전이다)**
> 지난 핸드오프가 "08-17 첫 실전" 이라고 적었는데 **그날은 아직 오지 않았다**(이 세션은
> 08-16 밤). 확인 목록은 그대로 유효하고 여기에 셋이 붙는다:
> ⑤ 리뷰 카드가 reviews.html 에 실제로 뜨는지 ⑥ 예고 버튼이 실제로 눌리는지(식별자·입력
> 계약 수정 후 첫 실전) ⑦ 순차진행을 켰을 때 하루 한 트랙만 도는지 ⑧ 트랙 토글을 껐을 때
> 정말 안 나가는지(종전에는 버튼이 거짓말을 했다).
>
> **■ 도구 메모** — 컨테이너의 Codex 토큰이 만료돼 있었다(refresh 재사용). `codex login
> --device-auth` 로 **폰만으로** 갱신된다 — 정본 문서(§3-1)의 "데스크탑 2분" 은 옛 경로다.

> **[2026-08-16(3) — 배포 완료 · ★ PeterJ 새 요구 5건 (다음 세션 착수점)]**
>
> **■ ★★ 다음 세션은 이 5건으로 시작한다** (PeterJ 지시 2026-08-16 마무리 직전).
> 지금 구조를 상당히 바꾸는 요구다. **착수 전에 5건을 통으로 읽고 설계부터 잡아라** —
> 하나씩 따로 붙이면 페이지 구조가 두 번 갈린다.
>
> **① 2페이지 → 3페이지로 가른다**
> 지금은 `index.html`(논문) + `guidelines.html`(가이드라인·기타) 둘이다.
> 이것을 **논문 / 가이드라인 / 리뷰·기타** 셋으로 나눈다.
> **누적 리스트도 각 페이지에 맞춰** 나눈다 — 리뷰 표를 새로 만들고 "기타" 를 리뷰 쪽으로 옮긴다.
> 관련 코드: `src/utils/pageSplit.js`(`mergePages`/`splitPages`) · `GitHubPublisher._renderGuidelineState`.
>
> **② 예고 리스트를 각 페이지 맨 위로**
> 지금은 `index.html` 한 곳에만 세 트랙이 섞여 있다. **각 페이지에 그 트랙 것만** 맨 위에 놓는다.
> 삭제·발행 버튼도 거기 바로 붙인다.
> 관련 코드: `GitHubPublisher._renderUpcomingFromDisk`(지금 tracks 3개를 한 번에 넘긴다 —
> 페이지별로 갈라 부르게 고친다).
>
> **③ 세 페이지 모두 기본 접힘**
> **오늘 논문까지 포함해 전부 접힘**으로 시작한다(지금은 오늘 것이 펼쳐져 있다).
>
> **④ ★ 가이드라인 주기를 바꾼다 — "30일에 1편" 은 아니다**
> PeterJ 의도: *"아카이빙 캐치까지 매일 누적"*. 지금 큐에 6건뿐인 것은 **주 1회 게이트**
> 때문이지 공급이 없어서가 아니다(관심주제 지침은 연 2,888편 규모).
> 할 일: 게이트를 **매일**로 바꾸거나, PeterJ 가 on-demand 로 매일 돌릴 수 있게 하거나, 둘 다.
> `guidelineIntervalDays`(기본 7)가 그 손잡이다.
> **최종 그림(PeterJ 구상)**: 논문 → 가이드라인 → 리뷰 순으로 **하루 하나씩 · 3일 한 사이클**.
> 다만 "일단 기본 30일에 1개는 아니다" 까지가 확정이고 3일 사이클은 아직 구상이다.
>
> **⑤ 누적 리스트도 접힘 토글 · 전부 접힘이 기본**
>
> **■ 이번에 배포까지 끝낸 것**
> 예고 블록이 **실제 배포 페이지에 올라가 있다**(PR #108). 렌더 경로만 적용했고
> 아무것도 안 지워진 것을 숫자로 확인했다(SECTION 43→43 · 논문 표 88→88 · 553KB→564KB).
> **예고에 논문 줄이 없는 것은 정상이다** — `queue_papers.json` 은 데일리가 수집할 때
> 채워지는데 새 코드로 아직 안 돌았다. 08-17 06:30 이후 채워진다.
>
> **■ ★ 테스트 헛검사 2건 (같은 부류가 오늘만 9건째)**
> 실물 무손실 검사가 **두 군데서 아무것도 안 지키고 있었다.**
> ①길이를 전체로 비교 — 예고 블록은 설계상 교체되므로 작아지는 게 정상인데 오판했다.
> ②`data-guideline="1"` 로 누적 표를 지키는 척했는데 그 행은 페이지 분할로
> `guidelines.html` 에 있어 **index.html 에는 0개**였다. `0 >= 0` 이라 행을 통째로
> 지우는 변이가 **초록이었다.** 이제 **기준값이 0이면 테스트가 실패한다.**
> → ①번 작업(3페이지 분할)을 할 때 이 검사들이 다시 헛돌기 쉽다. 페이지가 갈리면
> 마커가 어느 파일에 있는지가 또 바뀐다. **기준값 0 가드를 믿되 확인하라.**

> **[2026-08-16(2) — 3트랙 파이프라인 S1~S8 구현 완료 (PR #106)]**
>
> **■ 무엇이 생겼나** — PeterJ 스케치("미리 풀링해두고 나는 배제만") 를 실제로 세웠다.
> ```
>   S1 트랙1 예비 큐 저장      매일 걷고 버리던 32~36편 풀을 남긴다
>   S2 트랙3 저수지 400편      5년 창 · 연도 가중(≤16.7%) · 같은 저널 연속 3건 제한
>   S3 1주 예고 렌더           날짜를 저장하지 않고 계산한다
>   S4 버튼 4종               🗑삭제 · ▶구동 · 켜짐/꺼짐/격일 · 전체 갈아엎기
>   S5 리뷰 arm               데일리가 리뷰 큐 소비(주 1회 게이트)
>   S6 읽음 서버화             localStorage → output/read_state.json 직커밋(러너 0회)
>   S7 텔레그램               트랙별 진행상황 줄 + md 첨부(sendDocument)
>   S8 md 아카이빙             output/digests/ · YAML front-matter · LLM wiki 자료용
>   +  트랙2 지역 필터         미국·유럽·한국 발표 기관만 (PeterJ 확정)
> ```
>
> **■ ★★ 이 세션이 배운 것 — "테스트는 초록인데 실제로는 안 돈다" 가 7건 나왔다**
> 전부 유닛 테스트가 못 잡는 부류다. **모듈이 옳은 것과 그 모듈이 불리는 것은 다르다.**
> ```
>   ① 지역 판정 모듈이 파이프라인에 안 걸려 있었다 (08-15 에 만들어 census 에만 씀)
>      → 지금까지 브라질·일본 지침이 그대로 들어왔다. 배선을 지워도 515건 전부 초록이었다
>   ② 예고 렌더도 아무도 안 불렀다 — 같은 부류가 하루에 두 번
>   ③ 배선하며 import 누락 — `node --check` 통과 (ESM 은 미정의 식별자를 파싱 때 못 잡는다)
>   ④ 데일리 진입점이 없는 지역 변수 참조 — 러너 전용 파일이라 테스트가 경로를 안 탄다
>   ⑤ 코덱스 하청이 발행 장부를 안 봄 → 예고에 이미 읽은 논문이 뜰 뻔
>   ⑥ CSS 클래스명 불일치(.up-row vs up-item) → 행 스타일이 통째로 안 먹음. 눈으로만 보였다
>   ⑦ ★ output/* 가 gitignore 라 큐가 커밋 안 됨 → 저수지 400편이 매 실행 증발
> ```
> **처방(다음 세션도 지킬 것)**: 모듈을 새로 만들면 **그것을 실제로 부르는 경로를 호출하는
> 테스트**를 같이 박는다. 배선을 지우는 변이가 적색이 되는 것까지 확인한다.
> `node --check` 는 배선 검증이 아니다.
>
> **■ 곁들여 잡은 것** — 테스트가 프로덕션 `output/` 을 오염시키고 있었다(픽스처가 실제
> 큐에 들어가 예고에 빈 줄로 떴다). 이제 `saveTrackQueue` 가 테스트 중 `output/` 쓰기를
> 던진다. **새 상태 파일을 만들면 `.gitignore` 예외도 같이 넣어라** — 테스트가 잡아준다.
>
> **■ 판단을 뒤집은 것** — "예고가 비면 빈 상자를 만들지 말자" 를 폐기했다. 트랙을 전부
> 끄면 **토글까지 사라져 다시 켤 방법이 없어진다.** 비어도 블록은 그리고 "없다" 고 말한다.
>
> **■ 상태 파일 지도 (쓰는 주체별로 갈랐다 — 섞으면 데일리 커밋과 버튼 커밋이 충돌한다)**
> ```
>   러너가 쓴다   selected_papers.json(장부·불변) · selected_guidelines.json(트랙2 v2 큐)
>                queue_papers.json · queue_reviews.json
>   브라우저가 쓴다 control_state.json(온오프) · read_state.json(읽음)
...
>
> **■ 지금 이 세션이 멈춰 있는 자리 = PeterJ 결정 6건.** 답이 오면 바로 파이프라인 구축에
> 들어가면 된다. 코드는 다 깔려 있고, **필요한 숫자도 전부 실측했다.**
>
> **① 트랙3(리뷰 아티클) 시장조사 실측** — PubMed · SR·메타 제외 · 관심주제로 좁힌 값
> ```
>   저널 묶음                          1년   3년   5년   주당(5년)
>   NEJM·JAMA·Lancet·BMJ (4종)         60   143   217     0.8
>   + Intensive Care Med·Crit Care Med  128   291   408     1.6   ← 추천
>   + Ann Emerg Med·Chest·AJRCCM·Circ   269   637   958     3.7
> ```
> 주제 필터로 4종은 203→60(30%), 10종은 474→269(57%). EM/CCM 저널이 섞일수록 적중률이 오른다.
> 축 ⑥(전량)·⑦(주제 필터)은 `scripts/guideline-census.mjs` 에 있고 `guideline-census.yml`
> 을 `reviews: true` 로 디스패치하면 다시 잰다. **LLM 토큰 0.**
>
> **② 세 트랙의 성격이 다르다 — 같은 주기로 굴리면 안 된다**
> ```
>   ① 논문(arm F)   흐르는 물   매일 새로 온다. 공급 넉넉. 지금 그대로.
>   ② 가이드라인     가뭄       30일 가상재생 = 발행 1건·스킵 30일. "매일 시도, 비면 스킵"이 맞다.
>   ③ 리뷰          저수지     5년 창에 408편을 쌓아두고 꺼내 쓴다. 고갈되지 않는다.
> ```
>
> **③ 큐레이션 UX — 검토함(inbox)을 만들지 않는다 (PeterJ 확정)**
> 배포페이지에 **1주치 예고 리스트**를 게시한다. 놔두면 날짜대로 진행, 🗑 누르면 예비에서
> 자동 충원. *"너무 여러개 검토는 그손가고 귀찮. 지속가능성 떨어짐."* — 검토함 설계로 되돌리지 말 것.
>
> **④ PeterJ 결정 대기 6건** (세션이 번호 줄글로 냈고 답을 못 받은 채 끝났다)
> ```
>   1. 트랙3 저널 세트     6종(주1.6) / 4종(주0.8) / 10종(주3.7)        추천 6종
>   2. 데일리 배분         논문매일+리뷰주1~2+지침나오는날 / 3개씩 / 로테이션   추천 1번
>   3. 트랙3 창           5년 / 3년 / 1년                              추천 5년
>   4. 사전 랭킹          트랙3만 / 트랙2·3 / 안함                      추천 트랙3만
>   5. needsReview 1,412건  예고리스트에 섞기 / tier-1만 / 사전랭킹      추천 예고리스트
>   6. 빌드 순서          예고UI 먼저 / 트랙3 먼저                      추천 예고UI 먼저
> ```
> 근거: 6종 × 주1회면 공급이 수요의 1.6배라 🗑 눌러도 충원되고, 5년 창이면 예비 408편이라 마르지 않는다.
>
> **■ 이번 세션에 머지된 것**
> - **#101** 트랙3 census 축 ⑥ + **상태 v2 착지로 깨져 있던 테스트 6건 복구**(444/6 → 450/450)
> - **#102** census 축 ⑦(관심주제로 좁힌 리뷰 + 주당 환산)
> - #100 은 닫음(낡은 main 기반 · 사용량 파일 반복 충돌 → #101 로 대체)
>
> **■ ★ 함정 셋 — 다음 세션이 같은 데 빠지지 말 것**
> - **`output/selected_guidelines.json` 은 이제 v2 객체다**(2026-08-16 데일리에서 실제로 넘어갔다).
>   실물을 읽는 테스트는 반드시 `test/helpers/guidelineProduction.mjs` 를 거친다 — 배열로
>   직접 읽으면 그날로 6건이 터진다. 스키마가 또 바뀌어도 **고칠 자리는 그 파일 하나다.**
> - **이 컨테이너에서 PubMed 를 직접 못 친다.** 에이전트 프록시가 `eutils.ncbi.nlm.nih.gov`
>   CONNECT 를 403(정책 거부)으로 막는다. 우회하지 말고 **Actions 러너로 돌린다.**
> - **PubMed 는 키 없이 초당 3회다.** census 가 429 로 통째로 죽었다. 요청 간격 + 지수 백오프를
>   `fetchRetry()` 에 넣었고 esearch·esummary·efetch 셋 다 그 경로로 태웠다. 새 축을 추가할 때
>   raw `fetch` 를 쓰지 말 것.
>
> **■ 미착수로 남은 것** — lowConfidence 노출(끝내 손 못 댐) · Google refresh token 재발급
> (PeterJ 데스크탑 필요 · PR #64 재검증이 여기 막혀 있다) · G5 실제 학회 사이트 셀렉터(망 차단)

> **[2026-08-14(5) — 쿨다운 검증·프로덕션 적용 · 논문 F 전환 · 가이드라인 5건 확정]**
>
> **★ 이 블록은 정답이 아니라 대조 기준선이다.** 다음 세션은 문서를 믿지 말고 실제(main·Actions·config)
> 를 먼저 재확인하라 — 이번 세션에 **문서와 실물이 어긋난 것이 4건** 나왔다(아래 §문서 오류).
>
> **■ ① 쿨다운 효과 검증 완료 (run `31799362820` · `31800022680` · llm=0)**
> 배선 버그 수정 후 F,G 재생이 정상 동작했다. **쿨다운은 주제 축에만 듣는다:**
> ```
>                 주제군  cardiac  resp  sepsis  neuro  infect   저널종수  최다저널
> F  (쿨다운 없음)  3군      19      10      1      0      0        8종      13편
> G  (5일·-2.0)    5군      11      10      7      1      1        7종      11편
> H  (7일·-3.0)    5군      11      10      7      1      1        7종      11편
> I  (3일·-3.0)    5군      11      10      7      1      1        7종      11편
> K  (7일·-4.0)    5군      11      10      7      1      1        7종      11편
> J  (10일·-2.0)   5군      12      11      5      1      1        6종      10편
> ```
> - **파라미터는 3~7일 · -2.0~-4.0 구간에서 무의미하다** — G·H·I·K 가 주제 분포까지 완전히 같다.
>   날짜별 선택은 다른데(G-H 17/30) 30일 총합이 같은 자리에 수렴한다.
> - **10일은 오히려 나쁘다.** 등장 주제군이 5개뿐이라 창이 길면 전부 동시에 감점 상태가 되고
>   상대 순서가 원점수로 돌아가는 것으로 보인다(**추정** — 숫자로 확인된 것은 "10일이 더 나쁘다"까지).
> - **★ 저널 쏠림은 어떤 파라미터로도 안 풀린다**(8종→7종). 별도 장치가 필요하다. **미착수.**
> - F 기준선이 종전 실측(심정지 16·기도 10)과 다른 것은 **코퍼스를 그날 새로 걷기 때문**이다.
>   같은 런 안의 F 대 G 비교만 유효하다.
>
> **■ ② 쿨다운 프로덕션 적용 (커밋 `c52111f`)** — 파라미터 **5일·-2.0** 확정(가장 약한 값).
> `interests.json` 의 `topicCooldown` 두 숫자로 폰에서 조정. days·penalty 0 이면 완전히 꺼진다.
> - **★ 배포 직후 며칠은 무동작이다** — `selected_papers.json` 에 `topic` 이 없었다. 오늘부터
>   같이 적기 시작했고 5일치가 쌓이면 완전히 동작한다. **자기치유형이라 손댈 것 없다.**
> - **★ 이력 적재를 호출부가 아니라 `_stageAnalyze` 안에서 한다.** 처음엔 `run()` 이 조립해
>   넘겼는데, **그 인자를 지우는 변이를 걸었더니 테스트가 전부 초록이었다** — 쿨다운만 조용히
>   죽는 F1 과 같은 구조다. 길목으로 옮긴 뒤에야 적색이 됐다.
> - test:unit 248 → 260. 변이 3종(이력 미적재·topic 미기록·감점 건너뜀) 전부 적색 확인.
>
> **■ ③ 논문 선정 F 전환 (PeterJ 확정 "논문은 F로 손보고 일단 돌려보자 한동안")**
> 365일 12구간 · 월 100편 사전순위 → 월 top3 → **36편이 LLM 풀**. 쿨다운이 이미 붙어 있으므로
> **프로덕션은 실질적으로 F + 쿨다운 = arm G 로 돈다**(F 를 그대로 켜면 주제 쏠림 87%).
> - `config/collection.json` 의 `"mode": "monthly12"` **한 줄을 지우면 종전 경로로 완전 복귀**한다.
> - `selectMonthlyPool`·`monthlyBuckets` 를 `src/utils/monthlyPool.js` 로 옮기고 재생 하네스는
>   re-export 로 받는다. 재생 export 6종 전수 확인 — 하네스 동작 불변.
> - `rerankPool` 은 `months × keepPerMonth = 36` 을 **코드가 유도**한다. `RERANK_POOL` repo
>   변수(기본 20)보다 우선 — PeterJ 가 폰에서 변수 화면을 찾을 일이 없게.
> - **★ 폴백 두 겹** — ①월별 수집 실패/빈 결과 → 단일 경로(`stats.monthlyFallback`)
>   ②**수집은 성공했는데 월별 갈래에서 풀이 비는 경우** → 수집분 전체
>   (`stats.monthlySelectionFallback`). ②는 ①이 못 잡는다(수집은 성공했으므로).
>   `pubDate` 가 미래(ahead-of-print)거나 360일 창 밖이면 버킷에 안 담긴다.
> - test:unit 260 → 271. 변이 4종 전부 적색 확인.
>
> **■ ★★ main 머지 — PeterJ 승인 후 진행**
> 데일리는 06:30 KST 에 **main 기준**으로 돌므로 브랜치에 있는 동안은 아무것도 안 바뀐다.
> PeterJ 가 "PR 만들고 머지까지" 를 선택해 머지했다. **내일 06:30 데일리부터 F + 쿨다운으로 돈다.**
> 되돌리려면 `config/collection.json` 의 `"mode": "monthly12"` 한 줄을 지운다.
>
> **■ ④ 가이드라인 개편 — PeterJ 5건 전부 확정**
> **①-B + ①-C 둘 다**(PT 외 consensus·statement·focused update·recommendations 확대 +
> 승인 학회 사이트 큐) · **②-C**(주제 무매칭 tier-1 은 needsReview 보존) ·
> **③-C**(신판 새 카드 + 구판 superseded 배지) · **④-D**(매일 시도, 큐 비면 건너뜀) ·
> **⑤-A**(수동 URL 은 PeterJ 승인이 곧 공식성 — 도메인 검증 넣지 말 것).
> 설계 계획서: `docs/superpowers/plans/2026-08-14-guideline-selection-redesign-plan.md` (G0~G10).
> - **설계의 핵심**: 오탐을 점수로 막지 않는다. 기관명+유행 주제가 제목에 함께 있으면 지침
>   논평도 높은 점수를 받는다. **문서 성격 판정을 스코어링 앞에 두고**, 점수는 진짜
>   가이드라인 사이의 발행 순서에만 쓴다. 애매한 것은 needsReview 로 격리한다.
> - **배포 순서에 못 하나**: G4/G5(수집 확대)와 G7(자동 발행)을 **같은 커밋에 넣지 않는다.**
> - **미착수** — G0(회귀 보호)부터가 다음 세션 몫이다.
>
> **■ ★ 문서 오류 4건 (이번 세션에 실물 대조로 잡았다 — 같은 부류가 더 있을 수 있다)**
> 1. **가이드라인 검색창** — 핸드오프가 "30일 구간당 0~6편"을 검색창 설명처럼 적었으나
>    `collectGuidelines()` 는 기본 **365일**이다. 0~6편은 희소도 측정 단위였다.
> 2. **감점 코드 위치** — "`MetadataScorer.js:191-210, 245-287`"이 아니라 소아·비급성 감점은
>    **`444-465`(`_negativePenalty`)** 에 있다.
> 3. **`docs/codex-mcp-setup.md` 가 저장소에 없다** — `CLAUDE.md` 가 "codex 문서 불러와" 하면
>    이 파일을 열라고 하는데 실물이 없다. (`CLAUDE.md` 수정 필요 — **미착수**.)
> 4. **코덱스 하청 경로** — 룰북이 2026-08-13(GC PR #148)에 **CLI(`codex exec`) 직접 호출 +
>    출력은 파일로**로 개정됐는데 이 세션은 MCP 래퍼로 보냈다. PeterJ 지적으로 정정.
>
> **■ 코덱스 — CLI 는 되고 MCP 는 안 된다 (이 컨테이너 실측)**
> `codex exec --model gpt-5.6-sol` 정상 동작(스모크 `CLI_OK`, codex-cli 0.147.0, auth 유효).
> **MCP 서버 경로는 60초 타임아웃으로 실패**했다. 프록시가 `ab.chatgpt.com`(텔레메트리)
> CONNECT 에 403 을 반복 기록하지만 **CLI 는 그것과 무관하게 돈다.**
> → **하청은 CLI 로 보낼 것.** 이 세션은 하청 4건 중 2건을 CLI 로 성공시켰다.
>
> **■ 손대지 않은 것 (의도적)**
> - **재생 생존자 편향** — 코퍼스 구조 변경이라 무겁다. **저널 품질은 깎아 보고, 채움률
>   30/30 은 보수적인 값**이라는 해석은 유효하다.
> - **저널 쏠림 장치** — 쿨다운은 주제 축 전용임이 실측으로 확정됐다. 별도 설계 필요.
> - **`candidateTopicCap: 5`** — 코드 참조 0건인 죽은 설정. 그대로 뒀다.
>
> **[2026-08-14(4) — 세션 마감: IF 표기 + 주제 쿨다운 구현 · 쿨다운 효과는 미검증]**
>
> **★ MP 미갱신** — PeterJ 토큰 한도 때문에 이번 마무리에서 `docs/master-plan.html`(v29)
> 갱신·재게시를 **건너뛰었다**. 아티팩트 URL 은 그대로다(757a28f8…). 다음 세션이 MP 를 열면
> 이 세션 내용(census·arm E/F/G·IF·쿨다운·실버그 7건)이 아직 안 들어가 있다.
>
> **■ 만든 것 (전부 푸시됨 · main 무변경)**
> - **저널 IF 표기** — `journals.json` 에 `impactFactors` 57종(통상 알려진 근사치, 대략 2023 JCR).
>   `src/utils/journalMeta.js` 가 조회하고 리포트 본문·표·발행 카드 4곳에 붙는다.
>   **모르는 저널은 아무것도 안 찍는다**(틀린 IF 보다 낫다). 괄호 접미사는 티어와 같은 방식으로 정규화.
>   PeterJ 확정: "걍 정확하지 않아도 통상적으로 알려진 IF 적어봐" → 폰에서 숫자만 고치면 된다.
> - **주제 쿨다운** `src/utils/topicCooldown.js` + arm G(= F + 쿨다운 5일·-2.0).
>   **배제가 아니라 감점**이다 — 어제 주제라고 후보에서 빼면 그날 정말 큰 논문이 그 주제일 때 놓친다.
>   어제 전액 → cooldownDays 째 0 으로 선형 감쇠. days·penalty 0 이면 완전히 꺼진다.
>   **프로덕션 경로에는 아직 안 붙였다.**
>
> **■ ★ 쿨다운 효과는 아직 모른다 — F,G 재생이 배선 버그로 무효였다**
> `needMonthly = requested.includes('E')` 라 **F·G 를 요청하면 월별 코퍼스를 안 걷어
> 두 arm 다 30/30 빈손**이 나왔다. 쿨다운 문제로 보였지만 내 배선 버그였다.
> collection 모드로 판정하도록 고쳤고(`scripts/selection-experiment.mjs:78`), **재실행은 안 했다**.
> → **다음 세션 첫 스텝: `arms=F,G · llm=0` 재생 한 번.** 로그 끝 `=== COMPACT ===` 의
>   `주제 F:` / `주제 G:` 줄만 보면 된다(F 기준선: 심정지 16 · 기도 10 · 저널 8종 · 최다 12편).
>
> **■ 하청 2 (codex Sol · read-only) — 가이드라인 선정 진단 완료**
> 결과 전문: 세션 스크래치패드(휘발). **요지만 옮긴다:**
> - **자동 경로는 사실상 죽어 있다.** PubMed `Guideline/Practice Guideline` PT 가 필수인데
>   census 실측으로 30일 구간당 0~6편뿐이다. consensus·scientific statement·focused update 는
>   그 PT 가 없으면 통째로 빠진다(`DataCollectorAgent.js:500-520`).
> - **가이드라인에 논문용 스코어러가 그대로 걸린다** — 배제 저널·주제 게이트(-5)·설계 점수·
>   소아/비급성 감점까지. 전용 기준이 없다(`MetadataScorer.js:245-287, 191-210`).
> - **개정판 규정이 아예 없다.** 다른 PMID·다른 URL 의 신판은 완전히 별개 항목이다.
>   계보(lineage)·supersede 판정이 코드에 없다. 2026-08-06 재설계 스펙의 `GuidelineScorer`·
>   `guideline-orgs.json`·큐·`needsReview` 는 **저장소에 존재하지 않는다**(문서만 있다).
> - **문서와 코드 불일치**: 스펙은 "공식 발행기관 진료지침"이라는데 수동 URL 경로는 도메인·
>   기관 검증이 전혀 없다(`on-demand.mjs:38-43`).
>
> **■ ★ PeterJ 가 정할 것 (가이드라인 — 다음 세션 본안)**
> ① **자동 인정 범위**: PT 만(현행·희소) / consensus·statement 까지 확대(오탐 위험) /
>    PubMed + 승인 학회 사이트 큐(가장 빠르지만 기관 목록 유지 필요)
> ② **기관 권위 vs 주제 적합 우선순위**: 권위 우선 / 주제 우선 / 주제 무매칭 tier-1 은 `needsReview` 보존
> ③ **개정판**: 새 카드로 계속 / 신판이 구판 대체 / 신판 새 카드 + 구판에 `superseded` 배지
> ④ **발행 주기**: 고정 7일 / 중요 신판 즉시 + 백필 주 1회 / 큐 깊이 따라 2·3·7일
> ⑤ **수동 URL 의 공식성 책임**: PeterJ 선택을 최종 승인으로 볼지 / 도메인 허용목록을 둘지
>
> **■ 남은 결정 (풀 쪽)**
> - E vs F — 결정적으로 30/30 동일. 토큰 4.7배를 지불할 이유를 못 찾으면 **F 가 기본값**.
> - **F 를 그대로 켜면 안 된다** — 주제 쏠림 87%(2026-08-14(3) 블록). 쿨다운 검증이 선행.
> - 재생 생존자 편향 미해결 — **저널 품질은 깎아 보고, 채움률 30/30 은 보수적인 값**.
>
> **[2026-08-14(3) — ★ F 를 그대로 켜면 안 된다: 주제 쏠림 발견]**
> A~F 6개 arm 재생(run `31748419155`) 결과. **E-F 일치 30/30 — 예측대로 결정적으로 동일하다**
> (월별 top-10 합집합의 최고점은 그 달의 1위이므로 top-3 으로 줄여도 남는다).
> 리스트 아티팩트: https://claude.ai/code/artifact/5bc1961d-f9d2-442b-84de-c7b1e55606c5
>
> **■ F 가 A 보다 나은 것 (확실)**
> - **빈손 13/30 → 0.** 편향 없는 진짜 숫자다.
> - **주제 이탈이 준다.** A 가 고른 17편 중 **8편(47%)이 EM/CCM 코어 밖**이었다 —
>   `Evaluation of the Educational Quality…`(교육 콘텐츠 평가! 배제 대상 계열이 1위로 뽑혔다) ·
>   `Remnant Cholesterol Paradox`(지질 역학) · `28-Day Mortality in COPD`(호흡기 내과).
>
> **■ ★ 그런데 새 문제 — 주제·저널 쏠림 (E·F 공통, 다음 세션이 반드시 볼 것)**
> ```
> 주제            A(17편)   F(30편)
> 심정지·소생        2        16
> 기도·인공호흡      4        10
...
> 외상              2         0
> 저널 종수         11         8
> 최다 저널          3    12 (Resuscitation)
> ```
> **30일 중 26일이 심정지 아니면 인공호흡이다(87%).** 관심주제 10그룹인데 2그룹만 나온다.
> 원인: 일간 top-1 은 풀 전체의 argmax 인데 `cardiac_resus` 주제군 + Resuscitation(대표지 3.2)
> 조합이 거의 항상 이긴다. **풀을 1년으로 넓힐수록 그 조합이 더 쌓여 쏠림이 심해진다.**
> → **F 채택 시 주제 다양성 장치(예: 최근 N일 내 같은 주제군 감점 = 쿨다운)를 함께 넣어야 한다.**
> → 참고: `config/collection.json` 의 **`candidateTopicCap: 5` 는 코드 어디서도 안 쓴다(죽은 설정).**
>   원래 이 문제를 막으려던 자리로 보인다.
>
> **■ 비용 (실측 기준)**
> LLM 셀렉 30일: 현행 풀20 **428,220** · F 풀36 **606,033**(환산) · E 풀120 **2,020,110**(실측).
> → **F 는 현행의 1.4배**. 단 **PubMed 요청은 약 6배**(31 → 192회/일: esearch 12 + esummary 60
>   + efetch 120) — 토큰이 아니라 실행 시간·실패 지점이 는다.
> **36편 토큰은 환산값이다** — 20·120 만 직접 쟀다. F 를 한 번 돌리면 확정된다.
>
> **■ PeterJ 요청 (미착수) — 선정 논문 옆에 IF 표기**
> IF 는 Clarivate JCR 유료 데이터라 자동 수집 경로가 없다. `journals.json` 에 값을 넣고
> 폰에서 고치는 방식이 된다. **데이터 출처를 PeterJ 가 아직 안 정했다**(내 기억으로 채울지 /
> 자리만 만들지 / IF 대신 우리 티어를 적을지). 렌더 지점은
> `ReportGeneratorAgent.js:365` · `GitHubPublisher.js:151,224`.
>
> **[2026-08-14(2) — 분업모드 ON · 코덱스 하청 리뷰가 실버그 7건을 잡았다]**
> PeterJ 지시로 **분업모드 ON**, 이 세션은 코덱스 적극 활용.
> **하청 1**: `read/code 중간 이상` → **codex Sol**(CLI 직접 호출 · read-only · 출력은 파일로).
> 오늘 프로덕션 경로 diff 614줄(`src/` + `config/` 6파일) 실버그 검토.
> 판정: **치명(데일리를 깨뜨림) 없음** — 96KB 미만은 argv 경로 불변 · 풀 20 은
> `max(8192, 5800)` = 8,192 로 불변 · orchestrator 는 `run()` 만 부르니 월별 수집 진입점
> 자체가 분리 · `pubmedTa` 51종 URL 655자로 여유.
>
> **■ ★ 그중 하나는 내 검증 방식의 결함이었다 (다음 세션이 새길 것)**
> `education` 통패턴이 `Patient education and counseling`(임상 커뮤니케이션 연구지)까지
> 영구 배제하고 있었다. **나는 "정상 임상지 47종 오탐 0"이라고 보고했는데 그 47종을 내가
> 골랐다.** 반례가 목록에 없으니 당연히 통과했다 — **자기가 고른 목록으로 자기를 검증하면
> 이렇게 샌다.** 하청은 목록 밖에서 반례를 찾아왔다.
> → 구체 패턴 6종(`education and training` · `medical education` · `continuing education` ·
>   `education online` · `health sciences education` · `nursing education`)으로 좁혔다.
>   진짜 교육지(AEM Education and Training 등)는 계속 잡힌다. **반례를 테스트로 박았다.**
>
> **■ 고친 나머지 6건**
> - **월 경계 중복** — PubMed 날짜 범위는 양 끝 포함인데 이웃 구간의 end/start 가 같은 날이라
>   그날 논문이 두 달에 다 들어갔다(dedup 뒤 풀이 조용히 축소). 반개구간으로.
> - **`screenDepth` 절단이 조용했다** — 걸리면 그 달을 다 못 본 것이다. `truncated` 플래그+경고.
> - **월 top-K 뒤에 배제를 걸고 있었다** — 배제 저널이 10슬롯을 먹고 사라지면 그 달 기여가
>   10편 미만이 되고 밀려난 정상지는 보충 안 된다. 배제를 **앞으로**.
> - **`armDivergence` 가 후보 "수"만 비교** — 크기 같고 내용 다른 풀을 같다고 오판.  집합 비교로.
> - **★ "같은 논문을 고른 날" 지표 오보** — 고유 PMID 교집합을 "같은 날"이라고 적어
>   PeterJ 에게 3/30·1/30 으로 보고했다. 두 지표를 **분리해서** 낸다.
> - **E2BIG 임계가 문자 수** — 한글 7만 자면 UTF-8 21만 바이트라 임계를 통과해 놓고 터진다. 바이트로.
>
> **■ 앞서 고친 LLM 두 벽 (PeterJ 안이 돌려면 둘 다 필요했다)**
> - **출력 상한** — `submit_paper_scores` 는 편당 1엔트리라 출력이 풀에 비례한다. 실측
>   20편 → 3,883 = 편당 194. 고정 8,192 는 **42편이 한계**. `max(8192, min(64000, 1000+240×풀))`.
>   **바닥을 깐 이유**: 공식값만 쓰면 풀 20 에서 5,800 이라 데일리 예산이 오히려 줄었다.
> - **argv 상한** — 프롬프트를 argv 로 넘겨 리눅스 `MAX_ARG_STRLEN`(128KB)에 걸렸다.
>   120편 프롬프트 176,670자 → `spawn E2BIG`. 96KB 초과는 **stdin** 으로.
>
> **■ ★ 아직 안 고친 것 — 재생 생존자 편향 (E 숫자를 그대로 믿으면 안 된다)**
> arm E 코퍼스는 **수집 실행일 기준 월별 상위 100편**만 보존한다. 과거 재생일에서 미래
> 논문을 제거하고 나면 **당시 101위 이하를 복원할 수 없다** — 그래서 E 의 채움률·저널 품질이
> 실제보다 유리하게 나온다. 고치려면 월별 PMID/summary 를 통째로 보존하고 **재생일마다
> as-of 사전순위를 다시 계산**해야 한다(코퍼스 구조 변경이라 무겁다).
> **★ 편향의 방향은 둘로 갈린다 (2026-08-14 재검토 — 처음엔 둘 다 "유리하게"라고 적었는데 틀렸다):**
> - **저널 품질은 부풀려진다** — 코퍼스에 남은 논문은 전부 "한 달 경쟁에서 살아남은 상위 100편"이라
>   평균적으로 세다. **E·F 의 `최상위·대표지 100%` 는 깎아서 봐야 한다.**
> - **채움률은 오히려 불리하게 나온다** — 재생 날짜가 이를수록 그 명단에서 쓸 수 있는 편수가 적다.
>   즉 E·F 는 **얇은 후보로도 30/30 을 채운 것**이라 실제로는 더 쉽게 채운다. **30/30 은 보수적인 값이다.**
> - A 는 매일 새로 300편을 받으므로 이 편향이 없다 — **A 의 13일 빈손은 그대로 진짜다.**
>
> **PeterJ 판단 대기 중.**
>
> **[2026-08-14 — PeterJ 안(월별 풀) 시뮬레이션 · ★ rerank 출력상한 실버그]**
> **PeterJ 확정**: "최근 1년, 한 달씩 끊어 한 달 100편 스크리닝, 그중 스코어링으로 10편,
> 12달치 120편에서 한 편은 LLM 으로 선정." (arm E)
> **추가 확정**: "한 달을 자르는데 그 안에서 날짜 구분할 이유가 없다 — 날짜 최신순은
> 의미없는 컷오프." → 달 안에서 `sort=date` 금지, **점수로 100편을 고른다.**
>
> **■ ★★ 실버그 — 120편 rerank 는 지금 코드로 작동 자체를 못 했다 (조용히)**
> `submit_paper_scores` 는 **논문 1편당 1엔트리**를 요구하므로 출력이 풀에 비례한다.
> 실측: **20편 → 출력 3,883 토큰 = 편당 194.** 고정 상한 8,192 는 **42편이 한계**이고
> 120편은 약 23,300 토큰이 필요하다. 시뮬레이션에서 110편 호출이 **usage 0 으로 실패**했다.
> **더 나쁜 것은 조용하다는 점이다** — `_rerankSelect` 는 소프트 실패라 "결정적 순위 유지"로
> 넘어가므로 **LLM 이 안 돌았는데 돈 것처럼 보인다.** F1(4주 은폐)과 같은 부류다.
> - 고침: `_callLLM` 에 maxTokens 옵션, rerank 는 `min(64000, 1000 + 240×풀)` 로 요청.
>   캘리브레이션 표에 **실패 사유**를 같이 찍는다(`llmCalled` 는 "시도했다"이지 "성공했다"가 아니다).
> - **데일리(풀 20)는 상한 8,192 → 5,800 이지만 실측 사용량 3,883 이라 여유. 동작 불변.**
>
> **■ 1차 시뮬레이션 결과 (run 31720597379 · 날짜컷 버전 · 30일)**
> ```
> | 항목                    |    A(현행) |   E(PeterJ 안) |
> | 발행한 날               |   17 / 30 |      30 / 30  |
> | 일평균 LLM 풀           |      11.3 |        101.5  |
> | 30일 rerank 입력 토큰   |   182,279 |    1,543,618  |  ← 8.47×
> ```
> 같은 논문을 고른 날 **3/30**. **A 는 7/16~7/28 13일 연속 빈손, E 는 30일을 다 채웠다.**
> 토큰은 실측 — A 풀 20편 실호출로 29,811자 → 10,840 토큰(**2.75 chars/token**)을 재고
> 30일 총량은 일자별 실제 프롬프트 글자수 합 × 그 비율.
>
> **■ 만든 것**
> - `collectMonthlyPool()` — **2단 회수**. ①esearch 로 그 달 PMID 를 screenDepth(1000)까지
>   ②**esummary(docsum)로 싸게 훑어 사전순위**(저널 티어·설계·제목 히트)로 100편 선별
>   ③그 100편만 efetch. **날짜 절단이 점수 절단으로 바뀐다**(efetch 예산 1200 불변).
>   ★ 사전순위는 **주제 게이트를 끈다** — esummary 에 초록·MeSH 가 없어 켜면 제목에 관심어
>   없는 논문이 -5 로 깔린다(실측: 제목에 주제어 없는 NEJM RCT 가 게이트 켜면 1.00, 끄면 6.00).
>   배제 저널은 게이트 없이도 -8.25 로 걸러진다.
> - `monthlyBuckets`/`selectMonthlyPool` · `rerankPromptChars`(프로덕션과 같은 서식) ·
>   토큰 캘리브레이션(프로덕션 경로로 실제 호출) · A vs E 비교표.
> - 데일리 코어 무영향 — `includeMonthlyPool` 기본 false, arm E 요청 시에만 돈다.
>
> **■ 미확인 (다음 세션이 이어받을 것)**
> - **esummary 사전순위판 A vs E 재실행 결과를 아직 못 봤다**(커밋 `b191c9d` 로 돌렸다).
>   1차 결과는 **날짜컷 버전**이므로 그대로 인용하면 안 된다.
> - 1차의 월별 채움에서 **M11(330~360일)이 0** 이었다. 수집은 `pdat`, 버킷은 `pubDate`
>   (사실상 EDAT)라 축이 어긋난 탓으로 보인다 — 위 [2026-08-13(2)] 의 날짜축 불일치와 같은 뿌리.
>   **파서 pdat/edat 병기가 선행돼야 12구간이 제대로 찬다.**
>
...
- **[2026-08-29] 종합 카드 `kind=synthesis` 신설**: PR **#147** 머지. 문헌 2~5건을 한 장으로
  ...
  refId enum 고정 + 모르는 refId 를 버리는 렌더 필터. 실질 게이트(모든 축이 단일 입장이면
  요약 재탕 → 1회 재시도). 코드리뷰 9건 전부 반영 — **카드 유실 두 경로**(구성원 개별 발행이
  종합 카드를 지움 / 재실행 시 카드가 쌓임) 포함. 테스트 936 → **947 통과** · spec-lint 0 ·
  변이 **12종 전부 적색**.
- **[2026-08-29] 2026 심부전 지침 발행**: ▶ 로 개별 4건(ESC 2026 = URL 직접 지정 ·
  2022 AHA/ACC/HFSA · 보편정의 2판 PMID 42366993 · ACC HFpEF PMID 42494134) + **종합 1건**
  (PeterJ 지시로 2022 제외, 2026년 3종만 대조). run **33250759773** · 커밋 `24ff0dd` ·
  verify-pages 통과 · 가이드라인 **23건**. 종합 카드가 "두 틀을 조화시키는 지침은 이 묶음
  안에 없다" 와 "ACC HFpEF 초록 본문을 못 받아 2차 출처에 근거했다" 를 **스스로 적었다**.
- **[2026-08-17] 가이드라인/리뷰 트랙**: PR **#118~#121**(셀렉션 개편 — esummary 필드 오타·
  초록 미수집·PT 근거·▶ 승격·백필 apply / 질병명 188개 주제축 / LLM 셀렉 / 큐레이션 RSECTION),
  PR **#122~#124**(2년 풀 적립 · 소아 자동 배제 · 버튼 실버그 3건), PR **#125**(3트랙 분석 분리
  — 리뷰 `review` 번역 모드) 머지. 단위 테스트 **767건 통과** · spec-lint 통과.
  실측 큐: 가이드라인 672(queued 255) · 리뷰 397 · 논문 12.
- **[2026-08-17] 트랙3 리뷰 LLM 셀렉**: PR **#127** 머지. 리뷰 전용 프롬프트(가이드라인
  프롬프트 재사용 시 전건 격리) + `reviewRank` 정렬·필터 정본(종전 픽은 배열 머리를 집었다).
  벌크 실행 실측: 397건 전량 판정 · 채택 256 · 격리 141. 테스트 **782건 통과**.
- **[2026-08-17] ▶ 실버그**: PR **#129** 머지. 승격한 항목이 `llmFit.keep === false` 분기에
  걸려 **맨 뒤로** 가고 있었다(실측 258개 중 257·258위). PR #118 의 승격 수정을 PR #120 의
  적합도 정렬이 조용히 되돌린 자리 — 양쪽 테스트 다 초록이었다. 수동 승격이 무조건 1순위가
  되게 고쳤고 가이드라인 트랙도 같은 함정이라 같이 고쳤다. 테스트 **786건 통과**.
- **[2026-08-17] 삭제 반영 누락**: PR **#131 #132** 머지. 카드·표는 정상이었고 ⓐ아카이브
  저장 현황 ⓑ"지난 N건" 배지가 지우기 전 숫자를 말했다. 아카이브도 함께 지우게 바꾸고
  (재선정 방지 목록은 유지) 배지를 다시 세게 했다. 리뷰 삭제 경로의 남은 두 구멍(tag
  화이트리스트·targets)도 막았고, 워크플로 `git add` 누락을 계약 검사로 잠갔다.
  실측: 아카이브 42 → 14 · 배지 43 → 15. 테스트 **798건 통과**.
