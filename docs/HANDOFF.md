...
5. 마스터플랜 Artifact(대화형 계획서, HTML): **v13** = https://claude.ai/code/artifact/757a28f8-bef7-4d5d-bc38-0dbccf747a5f
   (R1~R4 완료·R5 대기·B5 폴백·4-Phase). **관리 규칙(PeterJ 지시 2026-07-07)**:
   ① 사용자향 항목은 **코드(R4·B5·A3 등) 쓰지 말고 쉬운 이름**으로 — 코드는 내부 문서(이 파일)에만.
   ② 마스터플랜 안에 **"버전 기록" 섹션을 누적**(덮어써도 문서에 역사가 남게) — 갱신 때마다
   최신 항목을 맨 위에 한 줄 추가하고 옛 항목은 지우지 않는다(플랫폼 버전 선택기가 옛 버전을
   일부만 보존하는 한계 보완). 갱신 시 라벨도 올릴 것(현재 v13-cleaned-cumulative).
