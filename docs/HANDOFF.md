...
- **절차**: ① PeterJ에게 구체 불만 항목 청취(디자인/내용/자막/구성 중 무엇이 어떻게) →
  ② 개선 계획을 짧게 제시·합의 → ③ 브랜치 새로 파서 작은 커밋 + spec-lint/테스트 →
  ④ PR CI 초록 → 병합 → ⑤ `video-sample.yml` 재트리거 → Artifacts 링크 전달 →
  ⑥ PeterJ 시청 → 피드백 반복. **승인 전 `ENABLE_VIDEO` 설정 금지.**
