...
## 공통 출력 포맷 (두 프롬프트 끝에 붙임 — 비교용 통일 뼈대)

```
아래 형식으로만 보고하라. 한국어로 쓰되 통계 수치·의학 용어 원문은 영어 유지 가능.
...
⚠️ 규칙: 위 수치·사실은 확인된 원문에 명시된 것만. 없으면 "원문에 미보고"라고 쓰고 지어내지 말 것.
```
