...
### ★★ 1-A-1. 선정과 발행을 가른다 — 예정리스트가 정본 (PeterJ 확정 2026-08-18)

> *"예비리스트에서 순서대로 안 뽑히면 예비리스트가 무슨 소용이 있니…"*
> *"예비리스트가 미리 선정 돌린 거로 보고 당일에는 예비리스트를 분석만 하는 걸로."*

**무엇이 문제였나** — 트랙2(가이드라인)·트랙3(리뷰)는 예정리스트에서 꺼내 쓰는데
**트랙1만 매일 PubMed 를 새로 뒤져 그 자리에서 뽑았다.** 예정리스트는 "이런 게 후보다"
를 보여 주는 장식이었다. 그래서 2026-08-18 에 **예정리스트에 없던 AHA 소생술 지침
문서가 "오늘의 논문" 으로 발행됐다**(Circulation · PMID 41122884). 화면이 "다음은
이것" 이라고 말하는데 실제로는 딴 게 나가는, 이 저장소가 반복해서 낸 사고의 가장 큰 판이다.

**현행 계약**

| 단계 | 언제 | 무엇을 |
|---|---|---|
| **선정(리필)** | 예정리스트 발행가능분이 **하한(7건) 밑**일 때만 | 수집 → 검증 → 풀 구성 → 점수 → 상위 14건을 예정리스트에 저장 (`_refillPaperQueue`) |
| **발행** | **매일** | 예정리스트 **머리 하나**를 꺼내 본문 확보 → PICO → 발행 → 큐에서 소진 (`_pickFromPaperQueue`) |

- **당일에는 수집도 LLM 재순위도 안 한다.** 선정은 리필 때 이미 끝났다.
- **순서 규칙은 배열 순서 하나뿐이다** — 정본 `src/utils/paperQueue.js`.
  논문 큐에는 `status` 도 `promotedAt` 도 붙이지 않는다(`queueControl` 이 일부러 안 붙이고
  회귀 테스트가 잠갔다 · PR #129). ▶ 는 항목을 배열 맨 앞으로 옮기는 것으로 동작한다.
- **★ 리필은 발행 뒤에 돈다.** 앞에 두면 오늘 채운 것이 오늘 나가서 "미리 선정" 이 아니게 된다.
- **★ 매일 리필하지 않는다.** 매일 수집·재순위를 돌리면 **어제 본 순서가 오늘 바뀐다** —
  그러면 "미리 선정해 둔 목록" 이라는 말이 다시 거짓이 된다.
- **★★ 폴백이 안전장치다** — 큐가 비었거나, 못 읽었거나, 예정 논문을 PubMed 에서 못
  받으면 **종전 수집·선정 경로로 떨어진다.** 이것이 없으면 큐가 마르는 날 발행이 통째로 멈춘다.
- 소진은 **`publish()` 앞**이다(publisher 가 큐 파일을 커밋한다). 분석 실패한 논문은
  소진하지 않는다 — 제외목록과 같은 이유로 다음 실행에서 재시도되게 남긴다.
- 계약 테스트 `test/paperQueueSelection.test.mjs` 가 **실제 `run()` 을 태워** 잠근다.

### 1-A-2. 논문 트랙에 지침 문서를 넣지 않는다 (2026-08-18)

정본 `src/utils/paperTypeFilter.js`. 판정은 **NLM PublicationType**(Guideline ·
Practice Guideline · Consensus Development Conference)이 1순위이고, 색인이 늦어 PT 가
아직 안 붙은 창을 덮기 위해 **좁은 제목 정형**(연도/편으로 시작 + guideline(s) +
발표 기관명)이 2순위다.
- **`Review` 는 배제 목록에 넣지 않는다** — 메타분석·체계적 고찰이 그 PT 를 달고 들어오고
  그것들은 원저 근거로서 가치가 크다. 넣으면 메타분석이 통째로 사라진다.
- **지침을 *연구한* 논문은 살린다**(준수도·비교·효과 RCT). 제목 정형이 그것을 가른다.
- 트랙2·3 에는 걸지 않는다 — 트랙1 전용이다.
...
- **주기 게이트는 없다 (확정 ④-D · G7)** — 가이드라인은 **매일 시도**하고 큐가 비면
  그날은 건너뛴다. `test/guidelineContract.test.mjs` 가 이것을 잠근다.
  > ★ 2026-08-16 정정. 이 자리에는 오래 "주기 게이트 7일 — `guidelineIntervalDays` 기본 7"
  > 이라고 적혀 있었고 코드에도 `_guidelineDue()` 가 남아 있었는데, **그 함수를 아무도
  > 부르지 않았다.** 문서와 죽은 코드가 서로를 보증하는 바람에 다음 세션이 "큐가 6건뿐인
  > 것은 주 1회 게이트 때문" 이라고 오진했다. 실제로 발행을 막던 것은 관찰 모드
  > (`ENABLE_GUIDELINE_AUTOPUBLISH`)다. 죽은 손잡이는 없느니만 못해서 지웠다.
- **트랙 주기의 정본은 `src/utils/trackCadence.js` 하나다 (PeterJ 확정 2026-08-16 · 4-A)**
  세 트랙 전부 **매일**(`{papers:1, guidelines:1, reviews:1}`). 예고 렌더의 `cadence` 도
  이 값에서 파생된다 — 따로 적으면 **화면이 "매일 나간다" 고 해놓고 실제로는 주 1회만
  나가는** 상태가 된다(2026-08-16 실측 결함 B2).
- **순차진행 (PeterJ 확정 2026-08-16)** — `control_state.json` 의 `sequential: true` 를 켜면
  논문 → 가이드라인 → 리뷰 순으로 **하루 한 트랙**만 돈다. 담당 트랙은 달력 일수의
  나머지로 정하므로 저장할 상태가 없고, **게이트와 예고가 같은 함수를 본다.**
  기본은 꺼짐이라 **데일리 코어 무영향 불변식은 그대로**이며, 켜는 것은 PeterJ 의
  명시적 선택이고 버튼 한 번으로 되돌린다.
...
**★ 빈 줄이 규격의 일부다 (PeterJ 확정 2026-08-18 — 사유는 가독성).**

**★★ 세 트랙을 다 싣는다 (PeterJ 확정 2026-08-18 · 2차 개정).**
종전에는 논문만 실려서 가이드라인·리뷰가 나갔는지 알 길이 대시보드를 여는 것뿐이었다.
트랙 블록 사이는 **빈 줄 하나**. 순서는 화면 탭과 같다(논문 → 가이드라인 → 리뷰).
**그날 나간 트랙만** 싣는다(빈 블록을 만들지 않는다).
...
- **논문이 0편인 날에도 보낸다** — 가이드라인·리뷰만 나간 날이 그날의 전부다.
  하나도 안 나간 날은 "오늘 발행된 것이 없습니다" 라고 적는다(빈 알림은 고장과 구분이 안 된다).
- **핵심만**: 헤더 / 날짜 / 트랙별(제목·저널·PMID) / 링크. 스크리닝 설명·점수·evidenceLevel·
  LLM 경로·메달(🥇) 등 부가 정보는 **넣지 않는다** (PeterJ 요청, 2026-07-03).
- **링크는 매 발송마다 반드시 포함**. 메신저가 자동 링크화하도록 `https://` 포함.
- **본문은 항상 1건이다 — 분할하지 않는다.** 200자 2건 분할 규칙은 **폐지**(2026-08-18).
  그 규칙은 카카오 나챗방 상한 200자에서 온 것인데 카카오는 2026-08-04에 폐지됐고,
  텔레그램 상한은 4096자다. 남겨 두면 긴 제목일 때 본문이 갈리면서 **위 빈 줄 배치가
  깨진다** — 규격과 정면으로 충돌한다. 상한(4096)을 넘길 때만 **제목 하나를 잘라**
  구조를 지킨다(구조 > 제목 전문). 구현: `reportMessage.buildReportMessages`.
- **진행상황은 별도 메시지**이고 채널이 **빈 줄로 이어 붙인다**(`join('\n\n')`).
  빌더가 옳아도 이 구분자가 `'\n'` 이면 진행상황이 📊 링크 줄에 달라붙는다 —
  `test/reportMessage.test.mjs` 가 채널 쪽 구분자까지 검사한다.

## 2-B. 그날 분석 md 첨부 (PeterJ 확정 2026-08-18)

*"그날 분석한 내용 **동일내용**으로 md파일로 추가제시 — 여차하면 텔레그램에서 눌러서
그날거는 바로 읽게."*

- 정본: `src/utils/dailyDigest.js` (`buildDailyDigest` · `dailyDigestFilename`).
  발송: `TelegramNotifier.sendDocument` — 텔레그램은 `.md` 를 앱 안에서 미리보기로 연다.
- **"동일내용" 이 규격이다 — 여기서 요약하지 않는다.** 카드가 그리는 축을 그대로 옮긴다.
  ...
  · 리뷰 = 본문 번역(절별) · 확보 수준(coverage) · 임상 적용 — **요약 축은 없다**(PR #125)
- **읽기 형식 (PeterJ 확정 2026-08-18)**: 트랙 제목은 `#`(h1), 항목 제목은 `##`(h2),
  리뷰 소절은 `###`. **트랙 사이는 빈 줄 5개 + 구분선 + 빈 줄 5개** — 텔레그램 md
  미리보기는 폭이 좁아 구분선 하나만으로는 어디서 트랙이 바뀌는지 눈에 안 들어온다.
- **리뷰 웹 보강 (2026-08-18)**: 원문을 못 구하면 검색 사다리(DOI 랜딩 → PMC →
  저널·학회 공개 요약 → 권위 2차 출처)를 타고, 그래도 얇으면 **1회만** 에스컬레이션한다
  (`REVIEW_THIN_BODY_CHARS`). 보강 절은 `origin:'augmented'` 로 표시되고 제목에
  `[웹 보강]` 접두와 출처 링크가 강제로 붙는다 — **원문이 말한 것과 섞이면 안 된다**
  (섞이는 순간 "Lancet 이 이렇게 말했다" 로 읽히는데 실제로는 딴 데서 온 문장이 된다).
  `coverage` 는 보강 여부와 무관하게 **논문 본문 기준으로 정직하게** 적는다.
  축을 줄이면 "첨부를 봤는데 화면과 다르다" 가 되고, 그러면 아무도 첨부를 안 믿는다.
- **논문 0편인 날에도 보낸다** — 가이드라인·리뷰만 나간 날이 그날의 전부다.
  (종전에는 논문 0편이면 진입점이 즉시 exit 0 이라 그런 날 알림이 통째로 없었다.)
- 첨부 실패는 **소프트** — 본문 리포트도 파이프라인도 막지 않는다.
- 별개 산출물인 `src/utils/trackDigest.js`(트랙당 파일 하나 · wiki 적재용)와 혼동하지 말 것.
...
- **헤더 부제 (2026-08-18 개정 · PeterJ 확정)**: `매일 자동 선정 · 논문 · 가이드라인 · 리뷰`.
  종전 `180일 · 300편 스크리닝 → 1편/일 선정` 은 **옛말이라 뺐다** — 두 숫자는 논문 트랙
  수집 설정으로는 여전히 사실이지만(§1 표), 화면이 그것을 **선정 방식의 전부인 양** 말했다.
  지금은 3트랙이고 가이드라인·리뷰는 2년치 풀에서 LLM 판정으로 꺼낸다.
  `spec-lint` 의 정본 표기 검사에서도 `180일`·`300편` 을 뺐다(문구 정정을 막고 있었다).
- **통계 칸은 페이지당 카운트 하나 + 최종 업데이트 (PeterJ 확정 2026-08-18)**.
  *"분석일수·선정논문 구분할 이유 없으니 페이지당 카운트 하나만. 최종 업데이트는 그대로."*
  index = `발행 논문`(`stat-papers-count`) · guidelines = `발행 가이드라인` · reviews = `발행 리뷰`.
  `stat-papers-count`·`stat-updated-time` 클래스는 **계약**이다(삭제 경로의 `recountStats`
  와 publish 의 갱신 정규식이 이 이름으로 찾는다).
  칸 **구성**을 다시 그리는 정본은 `pageSplit` 이다 — `_buildPageRaw` 는 페이지가 아예
  없을 때만 도므로, 거기서만 고치면 배포본은 영원히 옛 구성으로 남는다.
...
- **배포 검증 게이트**: push 성공 ≠ 사이트 반영. 파이프라인 잡 종료 후 별도
  ...
  - **★ 2026-08-18 — 버튼 경로에도 붙였다.** 종전에는 `daily-review.yml` **하나에만**
    붙어 있어서, 🗑·▶·큐제어로 push 한 것은 배포 검증 없이 끝났다. 실측: 삭제를 28번
    연달아 누르면 Pages 가 앞 빌드를 계속 취소하고 **마지막 빌드가 취소된 뒤로 push 가
    없으면 사이트가 계속 뒤처진다.** 이제 `curate-remove` · `queue-control` · `on-demand`
    에도 같은 검증 잡이 붙는다.
  - **★ concurrency 는 검증 잡에만 건다** (`group: verify-pages`, `cancel-in-progress: true`).
    28번 누르면 검증 잡도 28개지만 **의미 있는 것은 마지막 하나**(사이트의 최종 상태)다.
    ★★ **본 작업 잡에는 절대 걸지 마라** — GitHub 은 그룹당 대기 1개만 유지해서
    앞에 누른 삭제가 **조용히 취소된다**(실측: run #6 cancelled · #7 cancelled · 화면 무반응).
    "일관성" 을 이유로 통일하지 말 것. 방향이 반대인 것이 옳다.
  - **`cancelled` 는 `failure` 와 다르게 다룬다** — 취소된 런에는 "실패한 잡" 이 없어
    `rerun-failed-jobs` 가 403/422 로 거부되고, 종전 코드는 그것을 권한 문제로 읽어
    **고치는 대신 빨간 알림만** 보냈다. 취소는 `/rerun`(전체 재실행)으로 되살린다.
  - **생성 HTML 을 쓰는 워크플로에 `git pull --rebase` 금지** — 생성 HTML 은 양쪽이
    통째로 다시 쓰므로 **반드시 충돌**하고, 성공해도 낡은 스냅샷으로 남의 변경을 되돌린다.
    최신 main 으로 `reset --hard` 한 뒤 **멱등 변형을 처음부터 재적용**한다.
  - `test/workflowDeployGate.test.mjs` 가 위 다섯을 워크플로 텍스트 계약으로 잠근다.
  (근거: 2026-07-05 GitHub 측 일시 오류로 배포만 실패 → 리포트 링크가 전날 데이터 노출.)

## 4-A. 리스트 명칭 (PeterJ 확정 2026-08-18 · 전 채널 공통)

명칭이 없어서 사람도 코드도 문서도 제각기 부르고 있었다. **이 세 이름만 쓴다.**

| 명칭 | 무엇 | 화면 위치 | 코드 |
|---|---|---|---|
| **예정리스트** | 앞으로 발행될 것들 (▶ 지금 발행 · 🗑 빼기 · 트랙 on/off · 전체 갈아엎기) | 각 페이지 맨 위 | `_renderUpcoming*` · `UPCOMING:<track>` · `UPBTN` |
| **발행리스트** | 분석해서 올린 카드들 (오늘 것은 펼침, 지난 것은 접힘) | 페이지 본문 | `SECTION`/`GSECTION`/`RSECTION` · `foldPast` |
| **누적리스트** | 지금까지 나간 것 전부의 표 (읽음 · 🗑 삭제 · 🎬 자료화) | 페이지 아래 | `arch-table` · `TABLE_ROWS_*` · `CURATION_BLOCK` |

- **식별자는 안 바꾼다** — `UPCOMING`·`SECTION`·`ARCHIVE_START`·`TABLE_ROWS_*`·`arch-table`·
  `at-count` 등은 이미 배포된 HTML 을 증분 패치할 때 다시 찾는 **기계 계약**이다.
  바꾸면 조용히 깨진다. 바꾸는 것은 **사람이 읽는 문자열**뿐이다.
- **📦 분석 보관 현황**(`ARCHIVE_STATUS`)은 **누적리스트가 아니다** — 그 분석이 어디까지
  보관됐는지(OA본문·웹·초록만·PDF적재) 보여주는 내부용 패널이다. 종전 이름
  "아카이브 저장 현황" 이 누적리스트와 헷갈려서 바꿨다.
- 화면 문자열을 고쳤으면 **버전 마커를 올려라**(`CURATION_BLOCK`·`UPBTN`·`READBTN`).
  안 올리면 증분 패치가 건너뛰어 **배포 페이지에 영원히 안 들어간다.**
  `test/deployedClientBlocks.test.mjs` 가 생성기와 배포본의 마커 일치를 검사한다.
...
## 4-H. 배포 페이지 3분할 — 논문 / 가이드라인 / 리뷰·기타 (PeterJ 확정 2026-08-16)

설계 근거: `docs/superpowers/specs/2026-08-06-selection-guideline-redesign-design.md` §5.5-B,
`docs/superpowers/plans/2026-08-16-three-page-restructure-plan.md`.
...
- **2 → 3 이 된 이유 (PeterJ 확정 2026-08-16)**: 트랙3(리뷰)이 실제 발행 arm 이 되면서
  세 트랙이 대등해졌다. 종전 근거("기타는 소량이라 단독 페이지를 못 채운다")는
  **기타를 리뷰와 한 페이지에 묶어** 그대로 유지된다 — 빈 페이지는 생기지 않는다.
- **각 페이지 맨 위에 그 트랙의 예고 리스트와 버튼**을 둔다(요구 ②).
  마커는 `<!-- UPCOMING:<track> -->` 이고 `splitPages` 가 제 페이지로 옮긴다.
- **카드는 전부 접힘이 기본**이다 — **오늘 논문 포함**(요구 ③). 시각적 강조
  (`day-today`)는 남기고 펼침만 없앤다. 배포본에 남은 옛 `open` 은
  `pageSplit.collapseAllCards()` 가 한 번에 걷는다.
- **누적 표도 접힘 토글**이고 기본이 접힘이다(요구 ⑤). `<details class="arch-fold">` 는
  `.arch-table` **안쪽**에 둔다 — 바깥에 씌우면 `endOfTableContainer` 의 div 균형 계산이
  어긋나 표가 복제된다(실측 사고 이력 있음).
...
1. **합쳤다가 가른다.** `publish()` 는 읽을 때 `mergePages(index, guidelines, reviews)` 로 단일
   본문을 만들고, 기존 증분 로직(지침 중복 제거·TODAY 강등·날짜 행 교체·PMID dedup·
   큐레이션 재적용·통계 갱신)을 **종전과 동일한 입력**에 적용한 뒤, 끝에서
   `splitPages()` 로 세 파일을 기록한다. 로직을 세 벌로 만들지 않는 것이 요점이다.
2. **마이그레이션은 자동.** 없는 페이지는 merge 가 건너뛰고 split 이 과거 카드·행까지
   가른다(기타는 `guidelines.html` → `reviews.html` 로 옮겨간다). 별도 일회성 스크립트
   없음. 다만 **렌더 코드를 고친 날 화면을 바로 맞추려면** `scripts/apply-page-render.mjs`
   를 돌린다(분석·수집 없음, LLM 토큰 0, 무손실을 숫자로 확인하고 줄면 안 쓰고 죽는다).
3. **★ 표 행 속성 순서 계약** — 어기면 데일리가 조용히 깨진다:
   ...
   - 가이드·리뷰·기타 행만 `data-kind="guideline|review|reference"` 를 **`data-pmid` 뒤에** 단다.
...
4. **소프트 실패**: 하위 페이지 읽기 실패는 "없음"으로 보고 진행. 스캐폴드가
   아니면 분할하지 않고 `index.html` 만 종전대로 기록한다 — 분할이 데일리를 막지 않는다.
5. 아카이브 저장 현황(§4-E)은 논문 아카이브 기준이라 **`index.html` 에만** 둔다.
6. 큐레이션 스크립트는 표를 **전부 순회**한다(`reviews.html` 은 표가 둘 — 리뷰·기타).
7. **★ 무손실 검사는 페이지 귀속까지 잰다** (`test/threePageNoLoss.test.mjs`).
   마커마다 "어느 페이지에 있어야 하는가" 를 표로 못 박고, **기준값이 0이면 테스트가
   스스로 실패한다.** 분할이 늘 때마다 마커의 집이 바뀌므로 이 가드가 없으면 검사가
   조용히 헛돈다 — 2026-08-16 에 실제로 그랬다(`0 >= 0`).
   대리 마커(정적 UI 문구 등)는 쓰지 않는다. 기준값이 0이 아니어도 **보호 대상이
   전멸하는 변이를 못 잡는** 거짓 양성이 된다.
