...
if (!/^\d{4}-\d{2}-\d{2}(-m-[0-9]{1,9})?$/.test(sectionKey)) {
...
const patched = removeSectionFromHtml(html, { sectionKey, pmid });
await writeFile('index.html', patched, 'utf8');

console.log(patched === html
  ? `섹션 ${sectionKey}: 페이지에 이미 없음(멱등) — 숨김 목록만 갱신`
  : `섹션 ${sectionKey} 제거 완료${pmid ? ` (+표 행 ${pmid})` : ''}`);
