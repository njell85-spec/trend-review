...
const msgSrc = read('src/utils/reportMessage.js');
...
// 알림 메시지 정본 포함(문자열 리터럴만). 패턴은 스크리닝 설명 맥락 한정(오탐 방지).
const oldSpec = [
...
for (const [name, src] of [
  ['src/utils/GitHubPublisher.js', pub],
  ['index.html', idx],
  ['src/utils/reportMessage.js(문자열)', msgStr],
]) {
...
// ── 3) 알림 메시지: 검사 앵커 + 대시보드 링크 폴백 (REPORT_SPEC §2) ──────────
// 앵커(빌더 메서드)가 사라지면 "무검사 통과"가 아니라 lint 실패로 처리한다.
if (!/export\s+function\s+buildReportMessages\s*\(/.test(msgSrc)) {
  errors.push('src/utils/reportMessage.js: buildReportMessages 앵커 소실 — 개명/삭제 시 spec-lint도 함께 갱신할 것');
}
if (!msgStr.includes('github.io/trend-review')) {
  errors.push('src/utils/reportMessage.js: 대시보드 링크 폴백(github.io/trend-review) 없음 (REPORT_SPEC §2)');
}

// ── 4) 알림 메시지: 금지 장식 없음 — 파일 내 모든 문자열 리터럴 (REPORT_SPEC §2) ─
...
for (const [re, label] of [
  ...
  if (m) errors.push(`src/utils/reportMessage.js: 발송 문자열에 금지 장식 ${label} 포함 → "${m[0]}" (REPORT_SPEC §2 — 핵심 5줄만)`);
}
